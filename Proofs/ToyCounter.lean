...
private theorem runToyCounter_script (x : Counter32) (fuelTail : Nat) :
  ...
  simpa [Nat.add_assoc, Nat.add_comm, Nat.add_left_comm] using (by
    refine RunScript.next (st' := st2 x) ?_ ?_
    · simpa using step0_eval x
    refine RunScript.next (st' := st3 x) ?_ ?_
    · simpa using step1_eval x
    refine RunScript.next (st' := st4 x) ?_ ?_
    · simpa using step2_eval x
    refine RunScript.next (st' := st5 x) ?_ ?_
    · simpa using step3_eval x
    refine RunScript.next (st' := st6 x) ?_ ?_
    · simpa using step4_eval x
    refine RunScript.next (st' := st7 x) ?_ ?_
    · simpa using step5_eval x
    refine RunScript.next (st' := st8 x) ?_ ?_
    · simpa using step6_eval x
    refine RunScript.next (st' := st9 x) ?_ ?_
    · simpa using step7_eval x
    refine RunScript.next (st' := st10 x) ?_ ?_
    · simpa using step8_eval x
    refine RunScript.next (st' := st11 x) ?_ ?_
    · simpa using step9_eval x
    refine RunScript.next (st' := st12 x) ?_ ?_
    · simpa using step10_eval x
    refine RunScript.next (st' := st13 x) ?_ ?_
    · simpa using step11_eval x
    refine RunScript.halted (fuel := fuelTail) ?_
    simpa using step12_eval x)

private theorem runToyCounter_halt (x : Counter32) :
    VmState.run stubHost 50 (st1 x) = StepResult.halt (-1) ((st13 x).tryCommit).2 := by
  have hscript : RunScript stubHost 50 (st1 x) (Sum.inl (-1, st13 x)) := by
    simpa [Nat.add_assoc, Nat.add_comm, Nat.add_left_comm] using runToyCounter_script x 37
  have hrun :
      VmState.run stubHost 50 (st1 x) = VmState.finalizeHalt (-1) (st13 x) := by
    simpa using run_of_script_halt (host := stubHost) (fuel := 50) (st := st1 x) (st' := st13 x)
      (exitCode := -1) hscript
...
private theorem runToyCounter_halt_133 (x : Counter32) :
  ...
  have hscript : RunScript stubHost 133 (st1 x) (Sum.inl (-1, st13 x)) := by
    simpa [Nat.add_assoc, Nat.add_comm, Nat.add_left_comm] using runToyCounter_script x 120
  have hrun :
      VmState.run stubHost 133 (st1 x) = VmState.finalizeHalt (-1) (st13 x) := by
    simpa using run_of_script_halt (host := stubHost) (fuel := 133) (st := st1 x) (st' := st13 x)
      (exitCode := -1) hscript
  have hok : (st13 x).tryCommit.fst = true := st13_tryCommit_ok x
  rw [hrun]
  simpa using
...
private theorem runToyCounter_eval_outCell (x : Counter32) :
  ...
  have hRunRaw : VmState.runRaw stubHost 50 (st1 x) = StepResult.halt (-1) (st13 x) := by
    simpa using runRaw_of_script_halt (host := stubHost) (fuel := 50) (st := st1 x) (st' := st13 x)
      (exitCode := -1) hscript
  have hok : (st13 x).tryCommit.fst = true := st13_tryCommit_ok x
  have hRun :
