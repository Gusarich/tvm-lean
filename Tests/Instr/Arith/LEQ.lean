...
/-
LEQ branch-mapping notes (Lean + C++ reference):
- Lean analyzed file: `TvmLean/Semantics/Exec/Arith/Leq.lean`
- C++ analyzed file: `/Users/daniil/Coding/ton/crypto/vm/arithops.cpp`
  (`exec_cmp`, opcode registration in `register_int_cmp_ops` with mode `0x877`).
...
- C++: 6 branch points / 9 aligned terminal outcomes
  (`LEQ` registration to `exec_cmp(..., mode=0x877, quiet=false)`;
   explicit `check_underflow(2)`; two `pop_int` type checks; invalid-int (NaN) path;
   compare result class `{<,=,>}` mapped by mode to `{-1,-1,0}`).
...
- `intOv` from NaN propagation in non-quiet LEQ;
...
- NaN handling must throw `intOv` in non-quiet LEQ;
- deterministic gas threshold for `PUSHINT n; SETGASLIMIT; LEQ`.
-/

private def leqId : InstrId := { name := "LEQ" }

private def mkCase
    (name : String)
    (stack : Array Value)
    (program : Array Instr := #[.leq])
...
private def runLeqDirect (stack : Array Value) : Except Excno (Array Value) :=
  runHandlerDirect execInstrArithLeq .leq stack

private def runLeqDispatchFallback (stack : Array Value) : Except Excno (Array Value) :=
  runHandlerDirectWithNext execInstrArithLeq .add (VM.push (intV 777)) stack

private def leqSetGasExact : Int :=
  computeExactGasBudget .leq

private def leqSetGasExactMinusOne : Int :=
  computeExactGasBudgetMinusOne .leq

private def genLeqFuzzCase (rng0 : StdGen) : OracleCase × StdGen :=
    ...
    { name := "unit/finite-ordering-sign-and-boundary-cases"
      run := do
        ...
        for c in checks do
          ...
          let y := c.2.1
          let expected := c.2.2
          expectOkStack s!"{x}<={y}" (runLeqDirect #[intV x, intV y]) #[intV expected] }
    ,
    { name := "unit/nan-propagation-throws-intov"
      run := do
        expectErr "x-nan" (runLeqDirect #[.int .nan, intV 1]) .intOv
        expectErr "y-nan" (runLeqDirect #[intV 1, .int .nan]) .intOv
        expectErr "both-nan" (runLeqDirect #[.int .nan, .int .nan]) .intOv }
    ,
    { name := "unit/type-errors-follow-pop-order"
      run := do
        expectErr "pop-y-first-null" (runLeqDirect #[intV 1, .null]) .typeChk
        expectErr "pop-x-second-null" (runLeqDirect #[.null, intV 1]) .typeChk
        expectErr "both-non-int-y-first" (runLeqDirect #[.cell Cell.empty, .null]) .typeChk }
    ,
    { name := "unit/underflow-ordering-on-short-stacks"
      run := do
        expectErr "empty" (runLeqDirect #[]) .stkUnd
        expectErr "one-int" (runLeqDirect #[intV 1]) .stkUnd
        expectErr "one-non-int-underflow-first" (runLeqDirect #[.null]) .stkUnd }
    ,
    { name := "unit/dispatch/non-leq-falls-through"
      run := do
        expectOkStack "dispatch/fallback" (runLeqDispatchFallback #[]) #[intV 777] }
  ]
  oracle := #[
    ...
    mkCase "nan/y-via-pushnan-intov" #[intV 42] #[.pushInt .nan, .leq],
    mkCase "nan/x-via-pushnan-xchg-intov" #[intV 42] #[.pushInt .nan, .xchg0 1, .leq],
    mkCase "nan/both-via-double-pushnan-intov" #[] #[.pushInt .nan, .pushInt .nan, .leq],
    mkCase "gas/exact-cost-succeeds" #[intV 7, intV 5]
      #[.pushInt (.num leqSetGasExact), .tonEnvOp .setGasLimit, .leq],
    mkCase "gas/exact-minus-one-out-of-gas" #[intV 7, intV 5]
      #[.pushInt (.num leqSetGasExactMinusOne), .tonEnvOp .setGasLimit, .leq]
  ]
  fuzz := #[
    { seed := 2026020803
      count := 500
