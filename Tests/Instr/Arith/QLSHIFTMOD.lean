...
/-
QLSHIFTMOD branch-mapping notes (Lean + C++ analyzed sources):
- Lean analyzed files:
...
- Lean (QLSHIFTMOD specialization): 7 branch sites / 6 reachable terminal outcomes
  (finite `ok`; quiet-NaN `ok`; `stkUnd`; shift `typeChk`; y `typeChk`; x `typeChk`).
- C++ (`exec_shldivmod` runtime-shift + Q registration): 7 branch sites / 14 terminal outcomes.

Key risk areas covered:
- floor remainder semantics across sign combinations and shift boundaries (`0..256`);
- strict pop/error order (shift first, then `y`, then `x`) including quiet-range precedence;
- quiet NaN funnels for divisor-zero, NaN operands, and bad runtime shift;
- oracle serialization constraints (NaN/out-of-range encoded only via `PUSHINT` prelude);
- exact gas cliff for `PUSHINT n; SETGASLIMIT; QLSHIFTMOD`.
-/

private def qlshiftmodId : InstrId := { name := "QLSHIFTMOD" }

private def qlshiftmodInstr : Instr :=
  .arithExt (.shlDivMod 2 (-1) false true none)

private def mkCase
    (name : String)
    (stack : Array Value)
    (program : Array Instr := #[qlshiftmodInstr])
...
private def mkCaseFromIntVals
    (name : String)
    (vals : Array IntVal)
    (programSuffix : Array Instr := #[qlshiftmodInstr])
    (gasLimits : OracleGasLimits := {})
    (fuel : Nat := 1_000_000) : OracleCase :=
...
private def mkShiftInjectedCase
    ...
    (gasLimits : OracleGasLimits := {})
    (fuel : Nat := 1_000_000) : OracleCase :=
  mkCase name #[intV x, intV y] #[.pushInt shift, qlshiftmodInstr] gasLimits fuel

private def runQlshiftmodDirect (stack : Array Value) : Except Excno (Array Value) :=
  runHandlerDirect execInstrArithExt qlshiftmodInstr stack

private def runQlshiftmodDispatchFallback (stack : Array Value) : Except Excno (Array Value) :=
  runHandlerDirectWithNext execInstrArithExt .add (VM.push (intV 4296)) stack

private def qlshiftmodSetGasExact : Int :=
  computeExactGasBudget qlshiftmodInstr

private def qlshiftmodSetGasExactMinusOne : Int :=
  computeExactGasBudgetMinusOne qlshiftmodInstr
...
private def classifyQlshiftmod (x y shift : Int) : String :=
...
private def genQlshiftmodFuzzCase (rng0 : StdGen) : OracleCase × StdGen :=
  ...
  let (case0, rng2) :=
    if shape = 0 then
      ...
      let kind := classifyQlshiftmod x y shift
      ...
      let (y0, r3) := pickInt257Boundary r2
      let y := if y0 = 0 then 1 else y0
      let kind := classifyQlshiftmod x y 256
      (mkCase s!"/fuzz/shape-{shape}/ok/{kind}/shift-256-boundary" #[intV x, intV y, intV 256], r3)
      ...
      (mkCase s!"/fuzz/shape-{shape}/error-order/shift-range-does-not-mask-y-type"
        #[intV x, .null] #[.pushInt (.num 257), qlshiftmodInstr], r2)
    else if shape = 20 then
      let (y, r2) := pickNonZeroSigned257ish rng1
      (mkCase s!"/fuzz/shape-{shape}/error-order/shift-range-does-not-mask-x-type"
        #[.null, intV y] #[.pushInt (.num 257), qlshiftmodInstr], r2)
      ...
      run := do
        ...
        for c in checks do
          ...
          expectOkStack s!"/unit/direct/x={x}/y={y}/shift={shift}"
            (runQlshiftmodDirect #[intV x, intV y, intV shift]) #[intV r] }
    ,
    { name := "/unit/direct/quiet-nan-funnels"
      run := do
        expectOkStack "/unit/quiet/divzero/nonzero-over-zero"
          (runQlshiftmodDirect #[intV 5, intV 0, intV 3]) #[.int .nan]
        expectOkStack "/unit/quiet/divzero/zero-over-zero"
          (runQlshiftmodDirect #[intV 0, intV 0, intV 3]) #[.int .nan]
        expectOkStack "/unit/quiet/shift-negative"
          (runQlshiftmodDirect #[intV 5, intV 7, intV (-1)]) #[.int .nan]
        expectOkStack "/unit/quiet/shift-overmax"
          (runQlshiftmodDirect #[intV 5, intV 7, intV 257]) #[.int .nan]
        expectOkStack "/unit/quiet/shift-nan"
          (runQlshiftmodDirect #[intV 5, intV 7, .int .nan]) #[.int .nan]
        expectOkStack "/unit/quiet/nan-y"
          (runQlshiftmodDirect #[intV 5, .int .nan, intV 3]) #[.int .nan]
        expectOkStack "/unit/quiet/nan-x"
          (runQlshiftmodDirect #[.int .nan, intV 5, intV 3]) #[.int .nan]
        expectOkStack "/unit/quiet/nan-both"
          (runQlshiftmodDirect #[.int .nan, .int .nan, intV 3]) #[.int .nan] }
    ,
    { name := "/unit/error-order/underflow-type-and-range-precedence"
      run := do
        expectErr "/unit/error/underflow/empty" (runQlshiftmodDirect #[]) .stkUnd
        expectErr "/unit/error/underflow/one-arg" (runQlshiftmodDirect #[intV 1]) .stkUnd
        expectErr "/unit/error/underflow/two-args" (runQlshiftmodDirect #[intV 1, intV 2]) .stkUnd
        expectErr "/unit/error/type/shift-top-null" (runQlshiftmodDirect #[intV 1, intV 2, .null]) .typeChk
        expectErr "/unit/error/type/shift-top-cell"
          (runQlshiftmodDirect #[intV 1, intV 2, .cell Cell.empty]) .typeChk
        expectErr "/unit/error/type/y-second-null" (runQlshiftmodDirect #[intV 1, .null, intV 2]) .typeChk
        expectErr "/unit/error/type/x-third-null" (runQlshiftmodDirect #[.null, intV 1, intV 2]) .typeChk
        expectErr "/unit/error/order/both-non-int-y-before-x"
          (runQlshiftmodDirect #[.cell Cell.empty, .null, intV 2]) .typeChk
        expectErr "/unit/error/order/shift-type-before-y-type"
          (runQlshiftmodDirect #[intV 1, .null, .cell Cell.empty]) .typeChk
        expectErr "/unit/error/order/shift-range-does-not-mask-y-type"
          (runQlshiftmodDirect #[intV 1, .null, intV 257]) .typeChk
        expectErr "/unit/error/order/shift-range-does-not-mask-x-type"
          (runQlshiftmodDirect #[.null, intV 1, intV 257]) .typeChk }
    ,
    { name := "/unit/direct/pop-order-top-three-consumed-below-preserved"
      run := do
        expectOkStack "/unit/pop-order/below-null"
          (runQlshiftmodDirect #[.null, intV 7, intV 3, intV 1]) #[.null, intV 2]
        expectOkStack "/unit/pop-order/below-cell-quiet-shift-range"
          (runQlshiftmodDirect #[.cell Cell.empty, intV 7, intV 3, intV 257]) #[.cell Cell.empty, .int .nan] }
    ,
    { name := "/unit/dispatch/non-qlshiftmod-falls-through"
      run := do
        expectOkStack "/unit/dispatch/fallback" (runQlshiftmodDispatchFallback #[]) #[intV 4296] }
  ]
  oracle := #[
    ...
    mkCase "/error-order/shift-range-does-not-mask-y-type-via-program"
      #[intV 5, .null] #[.pushInt (.num 257), qlshiftmodInstr],
    mkCase "/error-order/shift-range-does-not-mask-x-type-via-program"
      #[.null, intV 5] #[.pushInt (.num 257), qlshiftmodInstr],
    ...
    mkCase "/gas/exact-cost-succeeds" #[intV 7, intV 3, intV 1]
      #[.pushInt (.num qlshiftmodSetGasExact), .tonEnvOp .setGasLimit, qlshiftmodInstr],
    mkCase "/gas/exact-minus-one-out-of-gas" #[intV 7, intV 3, intV 1]
      #[.pushInt (.num qlshiftmodSetGasExactMinusOne), .tonEnvOp .setGasLimit, qlshiftmodInstr]
  ]
  fuzz := #[
    { seed := 2026020848
      count := 700
      gen := genQlshiftmodFuzzCase }
  ]
