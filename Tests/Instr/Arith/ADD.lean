...
private def runAddDirect (stack : Array Value) : Except Excno (Array Value) :=
  ...
  let (res, st1) := (execInstrArithAdd .add (pure ())).run st0
  match res with
  | .ok _ => .ok st1.stack
  | .error e => .error e
...
private def addGasForInstr (instr : Instr) : Int :=
  match singleInstrCp0GasBudget instr with
  | .ok budget => budget
  | .error _ => instrGas instr 16

private def addSetGasNeed (n : Int) : Int :=
  addGasForInstr (.pushInt (.num n))
    + addGasForInstr (.tonEnvOp .setGasLimit)
    + addGasForInstr .add
    + implicitRetGasPrice

private def addSetGasFixedPoint (n : Int) : Nat → Int
  | 0 => n
  | k + 1 =>
      let n' := addSetGasNeed n
      if n' = n then n else addSetGasFixedPoint n' k

private def addSetGasExact : Int :=
  addSetGasFixedPoint 64 16

private def addSetGasExactMinusOne : Int :=
  if addSetGasExact > 0 then addSetGasExact - 1 else 0
