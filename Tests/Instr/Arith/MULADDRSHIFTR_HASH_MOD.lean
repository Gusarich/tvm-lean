...
namespace Tests.Instr.Arith.MULADDRSHIFTR_HASH_MOD

/-
MULADDRSHIFTR#MOD branch-mapping notes (Lean + C++ reference):

- Lean analyzed files:
  - `TvmLean/Semantics/Exec/Arith/Ext.lean`
    (`execInstrArithExt`, `.shrMod true true 3 0 false (some z)` hash-immediate path)
...
- C++ analyzed files:
  - `/Users/daniil/Coding/ton/crypto/vm/arithops.cpp`
    (`exec_mulshrmod`, add+mul with const shift and nearest rounding)
  - `/Users/daniil/Coding/ton/crypto/vm/stack.cpp`
    (`check_underflow`, `pop_int`, `push_int_quiet`)

Branch/terminal counts used for this suite
(`mul=true`, `add=true`, `d=3`, `roundMode=0`, `quiet=false`, `zOpt=some z`):
...
- nearest-rounding tie behavior with signed-variation and exact/non-exact matrices;
...
private def muladdrshiftrHashModId : InstrId := { name := "MULADDRSHIFTR#MOD" }

private def slashCaseName (name : String) : String :=
  if name.startsWith "/" then name else s!"/{name}"

private def mkMuladdrshiftrHashModInstr (shift : Nat) : Instr :=
  .arithExt (.shrMod true true 3 0 false (some shift))
...
private def mkShiftCase
    ...
    (gasLimits : OracleGasLimits := {})
    (fuel : Nat := 1_000_000) : OracleCase :=
  mkCase name stack #[mkMuladdrshiftrHashModInstr shift] gasLimits fuel
...
private def mkShiftInputCase
    ...
    (gasLimits : OracleGasLimits := {})
    (fuel : Nat := 1_000_000) : OracleCase :=
  let instr := mkMuladdrshiftrHashModInstr shift
...
private def runMuladdrshiftrHashModDirect
    (shift : Nat)
    (stack : Array Value) : Except Excno (Array Value) :=
  runHandlerDirect execInstrArithExt (mkMuladdrshiftrHashModInstr shift) stack

private def runMuladdrshiftrHashModDispatchFallback
    (stack : Array Value) : Except Excno (Array Value) :=
  runHandlerDirectWithNext execInstrArithExt .add (VM.push (intV 9413)) stack
...
private def muladdrshiftrHashModGasProbeInstr : Instr :=
  mkMuladdrshiftrHashModInstr 7

private def muladdrshiftrHashModSetGasExact : Int :=
  computeExactGasBudget muladdrshiftrHashModGasProbeInstr

private def muladdrshiftrHashModSetGasExactMinusOne : Int :=
  computeExactGasBudgetMinusOne muladdrshiftrHashModGasProbeInstr
...
private def classifyMuladdrshiftrHashMod (left right addend : Int) (shift : Nat) : String :=
  let tmp : Int := left * right + addend
  let quotient := rshiftPow2Round tmp shift 0
  let remainder := modPow2Round tmp shift 0
...
private def genMuladdrshiftrHashModFuzzCase (rng0 : StdGen) : OracleCase × StdGen :=
  ...
  let (case0, rng2) :=
    if shape = 0 then
      ...
      let kind := classifyMuladdrshiftrHashMod left right addend shift
      ...
      let kind := classifyMuladdrshiftrHashMod tmpSeed 1 0 1
      ...
      let kind := classifyMuladdrshiftrHashMod tmpSeed 1 0 2
      ...
      let kind := classifyMuladdrshiftrHashMod 0 right addend shift
      ...
      let (addend, rng3) := pickSigned257ish rng2
      let (shift, rng4) := pickShiftMixed rng3
      let kind := classifyMuladdrshiftrHashMod left 0 addend shift
      ...
      let (right, rng3) := pickSigned257ish rng2
      let (shift, rng4) := pickShiftMixed rng3
      let kind := classifyMuladdrshiftrHashMod left right 0 shift
      ...
      let kind := classifyMuladdrshiftrHashMod left right addend 256
      ...
      let (right, rng3) := pickSigned257ish rng2
      let (addend, rng4) := pickSigned257ish rng3
      let kind := classifyMuladdrshiftrHashMod left right addend 1
      ...
      let kind := classifyMuladdrshiftrHashMod left 1 0 shift
...
def suite : InstrSuite where
  id := muladdrshiftrHashModId
  unit := #[
    { name := "/unit/direct/nearest-rounding-sign-ties-and-output-order"
      run := do
        ...
        for check in checks do
          match check with
          | (left, right, addend, shift, quotient, remainder) =>
              expectOkStack s!"/unit/ok/left={left}/right={right}/addend={addend}/z={shift}"
                (runMuladdrshiftrHashModDirect shift #[intV left, intV right, intV addend])
    ...
    { name := "/unit/direct/boundary-shift256-and-pop-order"
      run := do
        ...
        expectOkStack "/unit/pop-order/lower-null-preserved"
          (runMuladdrshiftrHashModDirect 1 #[.null, intV 7, intV 3, intV 0])
          #[.null, intV 11, intV (-1)]
        expectOkStack "/unit/pop-order/lower-cell-preserved"
          (runMuladdrshiftrHashModDirect 1 #[.cell Cell.empty, intV (-7), intV 3, intV 0])
          #[.cell Cell.empty, intV (-10), intV (-1)] }
    ,
    { name := "/unit/error/underflow-type-and-pop-order"
      run := do
        expectErr "/unit/error/underflow/empty"
          (runMuladdrshiftrHashModDirect 1 #[]) .stkUnd
        expectErr "/unit/error/underflow/one-item"
          (runMuladdrshiftrHashModDirect 1 #[intV 1]) .stkUnd
        expectErr "/unit/error/underflow/two-items"
          (runMuladdrshiftrHashModDirect 1 #[intV 1, intV 2]) .stkUnd
        expectErr "/unit/error-order/underflow-before-type-two-non-int"
          (runMuladdrshiftrHashModDirect 1 #[.null, .cell Cell.empty]) .stkUnd
        expectErr "/unit/error/type/w-top-null"
          (runMuladdrshiftrHashModDirect 1 #[intV 1, intV 2, .null]) .typeChk
        expectErr "/unit/error/type/w-top-cell"
          (runMuladdrshiftrHashModDirect 1 #[intV 1, intV 2, .cell Cell.empty]) .typeChk
        expectErr "/unit/error/type/y-second-null"
          (runMuladdrshiftrHashModDirect 1 #[intV 1, .null, intV 3]) .typeChk
        expectErr "/unit/error/type/x-third-null"
          (runMuladdrshiftrHashModDirect 1 #[.null, intV 2, intV 3]) .typeChk
        expectErr "/unit/error-order/pop-w-before-y-when-both-non-int"
          (runMuladdrshiftrHashModDirect 1 #[intV 1, .cell Cell.empty, .null]) .typeChk }
    ,
    { name := "/unit/error/intov-from-overflow"
      run := do
        expectErr "/unit/intov/overflow-max-max-plus-max-shift1"
          (runMuladdrshiftrHashModDirect 1 #[intV maxInt257, intV maxInt257, intV maxInt257]) .intOv
        expectErr "/unit/intov/overflow-min-min-plus-min-shift1"
          (runMuladdrshiftrHashModDirect 1 #[intV minInt257, intV minInt257, intV minInt257]) .intOv
        expectErr "/unit/intov/overflow-max-min-plus-zero-shift1"
          (runMuladdrshiftrHashModDirect 1 #[intV maxInt257, intV minInt257, intV 0]) .intOv
        expectErr "/unit/intov/overflow-max-max-plus-zero-shift1"
          (runMuladdrshiftrHashModDirect 1 #[intV maxInt257, intV maxInt257, intV 0]) .intOv }
    ,
    { name := "/unit/opcode/decode-hash-immediate-sequence"
      run := do
        let instrShift1 := mkMuladdrshiftrHashModInstr 1
        let instrShift256 := mkMuladdrshiftrHashModInstr 256
    ...
    { name := "/unit/opcode/encode-invalid-immediate-range"
      run := do
        let invalidLow := mkMuladdrshiftrHashModInstr 0
        let invalidHigh := mkMuladdrshiftrHashModInstr 257
    ...
    ,
    { name := "/unit/dispatch/non-muladdrshiftr-hash-mod-falls-through"
      run := do
        expectOkStack "/unit/dispatch/fallback"
          (runMuladdrshiftrHashModDispatchFallback #[]) #[intV 9413] }
  ]
  oracle := #[
    ...
    mkCase "/gas/exact-cost-succeeds" #[intV 7, intV 3, intV 1]
      #[.pushInt (.num muladdrshiftrHashModSetGasExact), .tonEnvOp .setGasLimit, muladdrshiftrHashModGasProbeInstr],
    mkCase "/gas/exact-minus-one-out-of-gas" #[intV 7, intV 3, intV 1]
      #[.pushInt (.num muladdrshiftrHashModSetGasExactMinusOne), .tonEnvOp .setGasLimit, muladdrshiftrHashModGasProbeInstr]
  ]
  fuzz := #[
    { seed := 2026020899
      count := 700
      gen := genMuladdrshiftrHashModFuzzCase }
  ]

initialize registerSuite suite

end Tests.Instr.Arith.MULADDRSHIFTR_HASH_MOD
