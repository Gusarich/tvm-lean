...
/-
NEQ branch-mapping notes (Lean + C++ reference):
- Lean analyzed file: `TvmLean/Semantics/Exec/Arith/Neq.lean`
- C++ analyzed file: `/Users/daniil/Coding/ton/crypto/vm/arithops.cpp`
  (`exec_cmp`, opcode registration in `register_int_cmp_ops` with mode `0x787`).
...
- C++: 6 branch points / 9 aligned terminal outcomes
  (`NEQ` registration to `exec_cmp(..., mode=0x787, quiet=false)`;
   explicit `check_underflow(2)`; two `pop_int` type checks; invalid-int (NaN) path;
   compare result class `{<,=,>}` mapped by mode to `{-1,0,-1}`).
...
- `intOv` from NaN propagation in non-quiet NEQ;
- out-of-gas at exact-minus-one budget.

Key risk areas:
- comparator truth-table mapping (`<` and `>` both return `-1`, `=` returns `0`);
- operand pop order (`y` then `x`) and corresponding error ordering;
- underflow-vs-type ordering for one-item stacks;
- NaN handling must throw `intOv` in non-quiet NEQ;
- deterministic gas threshold for `PUSHINT n; SETGASLIMIT; NEQ`.
-/

private def neqId : InstrId := { name := "NEQ" }

private def mkCase
    (name : String)
    (stack : Array Value)
    (program : Array Instr := #[.neq])
...
private def runNeqDirect (stack : Array Value) : Except Excno (Array Value) :=
  runHandlerDirect execInstrArithNeq .neq stack

private def runNeqDispatchFallback (stack : Array Value) : Except Excno (Array Value) :=
  runHandlerDirectWithNext execInstrArithNeq .add (VM.push (intV 777)) stack

private def neqSetGasExact : Int :=
  computeExactGasBudget .neq

private def neqSetGasExactMinusOne : Int :=
  computeExactGasBudgetMinusOne .neq

private def genNeqFuzzCase (rng0 : StdGen) : OracleCase × StdGen :=
    ...
    { name := "unit/finite-ordering-sign-and-boundary-cases"
      run := do
        ...
        for c in checks do
          ...
          let y := c.2.1
          let expected := c.2.2
          expectOkStack s!"{x}!={y}" (runNeqDirect #[intV x, intV y]) #[intV expected] }
    ,
    { name := "unit/nan-propagation-throws-intov"
      run := do
        expectErr "x-nan" (runNeqDirect #[.int .nan, intV 1]) .intOv
        expectErr "y-nan" (runNeqDirect #[intV 1, .int .nan]) .intOv
        expectErr "both-nan" (runNeqDirect #[.int .nan, .int .nan]) .intOv }
    ,
    { name := "unit/type-errors-follow-pop-order"
      run := do
        expectErr "pop-y-first-null" (runNeqDirect #[intV 1, .null]) .typeChk
        expectErr "pop-x-second-null" (runNeqDirect #[.null, intV 1]) .typeChk
        expectErr "both-non-int-y-first" (runNeqDirect #[.cell Cell.empty, .null]) .typeChk }
    ,
    { name := "unit/underflow-ordering-on-short-stacks"
      run := do
        expectErr "empty" (runNeqDirect #[]) .stkUnd
        expectErr "one-int" (runNeqDirect #[intV 1]) .stkUnd
        expectErr "one-non-int-underflow-first" (runNeqDirect #[.null]) .stkUnd }
    ,
    { name := "unit/dispatch/non-neq-falls-through"
      run := do
        expectOkStack "dispatch/fallback" (runNeqDispatchFallback #[]) #[intV 777] }
  ]
  oracle := #[
    mkCase "ok/order/zero-eq-zero" #[intV 0, intV 0],
    mkCase "ok/order/pos-ne-pos" #[intV 2, intV 1],
    mkCase "ok/order/pos-ne-pos-reverse" #[intV 1, intV 2],
    mkCase "ok/order/neg-eq-neg" #[intV (-7), intV (-7)],
    mkCase "ok/order/neg-ne-neg" #[intV (-7), intV (-8)],
    mkCase "ok/order/neg-ne-neg-reverse" #[intV (-8), intV (-7)],
    mkCase "ok/order/pos-ne-neg" #[intV 1, intV (-1)],
    mkCase "ok/order/neg-ne-pos" #[intV (-1), intV 1],
    ...
    mkCase "nan/y-via-pushnan-intov" #[intV 42] #[.pushInt .nan, .neq],
    mkCase "nan/x-via-pushnan-xchg-intov" #[intV 42] #[.pushInt .nan, .xchg0 1, .neq],
    mkCase "nan/both-via-double-pushnan-intov" #[] #[.pushInt .nan, .pushInt .nan, .neq],
    mkCase "gas/exact-cost-succeeds" #[intV 7, intV 5]
      #[.pushInt (.num neqSetGasExact), .tonEnvOp .setGasLimit, .neq],
    mkCase "gas/exact-minus-one-out-of-gas" #[intV 7, intV 5]
      #[.pushInt (.num neqSetGasExactMinusOne), .tonEnvOp .setGasLimit, .neq]
  ]
  fuzz := #[
    { seed := 2026020809
      count := 500
