...
/-
CMP branch-mapping notes (Lean + C++ reference):
- Lean analyzed file: `TvmLean/Semantics/Exec/Arith/Cmp.lean`
- C++ analyzed file: `/Users/daniil/Coding/ton/crypto/vm/arithops.cpp`
  (`exec_cmp`, opcode registration in `register_int_cmp_ops` with mode `0x987`).
...
- Lean: 7 branch points / 10 terminal outcomes
  (opcode dispatch; explicit two-arg underflow guard; two `popInt` failure points;
   `x/y` NaN-vs-finite split; finite compare split (`a < b`, `a = b`, `a > b`);
   non-quiet NaN push path).
- C++: 7 branch points / 10 aligned terminal outcomes
  (`CMP` registration to `exec_cmp(..., mode=0x987, quiet=false)`;
...
- finite success (`-1`, `0`, `1`);
- `stkUnd` (empty/one-arg);
- `typeChk` at first (`y`) and second (`x`) pop;
- `intOv` from NaN propagation in non-quiet CMP;
- out-of-gas at exact-minus-one budget.

Key risk areas:
- exact `< / = / >` result mapping to `{-1,0,1}`;
- operand pop order (`y` then `x`) and corresponding error ordering;
- underflow-vs-type ordering for one-item stacks;
- NaN handling must throw `intOv` in non-quiet CMP;
- deterministic gas threshold for `PUSHINT n; SETGASLIMIT; CMP`.
-/

private def cmpId : InstrId := { name := "CMP" }

private def mkCase
    (name : String)
    (stack : Array Value)
    (program : Array Instr := #[.cmp])
...
private def runCmpDirect (stack : Array Value) : Except Excno (Array Value) :=
  runHandlerDirect execInstrArithCmp .cmp stack

private def runCmpDispatchFallback (stack : Array Value) : Except Excno (Array Value) :=
  runHandlerDirectWithNext execInstrArithCmp .add (VM.push (intV 777)) stack

private def cmpSetGasExact : Int :=
  computeExactGasBudget .cmp

private def cmpSetGasExactMinusOne : Int :=
  computeExactGasBudgetMinusOne .cmp

private def genCmpFuzzCase (rng0 : StdGen) : OracleCase × StdGen :=
    ...
    { name := "unit/finite-ordering-sign-and-boundary-cases"
      run := do
        ...
        for c in checks do
          ...
          let y := c.2.1
          let expected := c.2.2
          expectOkStack s!"cmp/{x}/{y}" (runCmpDirect #[intV x, intV y]) #[intV expected] }
    ,
    { name := "unit/nan-propagation-throws-intov"
      run := do
        expectErr "x-nan" (runCmpDirect #[.int .nan, intV 1]) .intOv
        expectErr "y-nan" (runCmpDirect #[intV 1, .int .nan]) .intOv
        expectErr "both-nan" (runCmpDirect #[.int .nan, .int .nan]) .intOv }
    ,
    { name := "unit/type-errors-follow-pop-order"
      run := do
        expectErr "pop-y-first-null" (runCmpDirect #[intV 1, .null]) .typeChk
        expectErr "pop-x-second-null" (runCmpDirect #[.null, intV 1]) .typeChk
        expectErr "both-non-int-y-first" (runCmpDirect #[.cell Cell.empty, .null]) .typeChk }
    ,
    { name := "unit/underflow-ordering-on-short-stacks"
      run := do
        expectErr "empty" (runCmpDirect #[]) .stkUnd
        expectErr "one-int" (runCmpDirect #[intV 1]) .stkUnd
        expectErr "one-non-int-underflow-first" (runCmpDirect #[.null]) .stkUnd }
    ,
    { name := "unit/dispatch/non-cmp-falls-through"
      run := do
        expectOkStack "dispatch/fallback" (runCmpDispatchFallback #[]) #[intV 777] }
  ]
  oracle := #[
    ...
    mkCase "nan/y-via-pushnan-intov" #[intV 42] #[.pushInt .nan, .cmp],
    mkCase "nan/x-via-pushnan-xchg-intov" #[intV 42] #[.pushInt .nan, .xchg0 1, .cmp],
    mkCase "nan/both-via-double-pushnan-intov" #[] #[.pushInt .nan, .pushInt .nan, .cmp],
    mkCase "gas/exact-cost-succeeds" #[intV 7, intV 5]
      #[.pushInt (.num cmpSetGasExact), .tonEnvOp .setGasLimit, .cmp],
    mkCase "gas/exact-minus-one-out-of-gas" #[intV 7, intV 5]
      #[.pushInt (.num cmpSetGasExactMinusOne), .tonEnvOp .setGasLimit, .cmp]
  ]
  fuzz := #[
    { seed := 2026020807
      count := 500
