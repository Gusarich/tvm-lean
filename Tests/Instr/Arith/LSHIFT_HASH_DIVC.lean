...
/-
LSHIFT#DIVC branch-mapping notes (Lean + C++ reference):

- Lean analyzed files:
  - `TvmLean/Semantics/Exec/Arith/Ext.lean`
    (`execInstrArithExt`, `.shlDivMod 1 1 false false (some z)` specialization)
...
Branch/terminal counts used for this suite (`d=1`, `roundMode=1`, `add=false`,
...
- ceil rounding for sign-mixed and near-zero inexact `(x << z) / y`;
...
- exact gas cliff for `PUSHINT n; SETGASLIMIT; LSHIFT#DIVC`.
-/

private def lshiftHashDivcId : InstrId := { name := "LSHIFT#DIVC" }

private def slashCaseName (name : String) : String :=
  if name.startsWith "/" then name else s!"/{name}"

private def mkLshiftHashDivcInstr (shift : Nat) : Instr :=
...
private def mkShiftCase
    ...
    (gasLimits : OracleGasLimits := {})
    (fuel : Nat := 1_000_000) : OracleCase :=
  mkCase name stack #[mkLshiftHashDivcInstr shift] gasLimits fuel
...
private def mkShiftInputCase
    ...
    (gasLimits : OracleGasLimits := {})
    (fuel : Nat := 1_000_000) : OracleCase :=
  let instr := mkLshiftHashDivcInstr shift
  let (stack, progPrefix) := oracleIntInputsToStackOrProgram vals
  mkCase name stack (progPrefix.push instr) gasLimits fuel

private def runLshiftHashDivcDirect (shift : Nat) (stack : Array Value) : Except Excno (Array Value) :=
  runHandlerDirect execInstrArithExt (mkLshiftHashDivcInstr shift) stack

private def runLshiftHashDivcDispatchFallback (stack : Array Value) : Except Excno (Array Value) :=
  runHandlerDirectWithNext execInstrArithExt .add (VM.push (intV 5517)) stack
...
private def lshiftHashDivcGasProbeInstr : Instr :=
  mkLshiftHashDivcInstr 7

private def lshiftHashDivcSetGasExact : Int :=
  computeExactGasBudget lshiftHashDivcGasProbeInstr

private def lshiftHashDivcSetGasExactMinusOne : Int :=
  computeExactGasBudgetMinusOne lshiftHashDivcGasProbeInstr
...
private def genLshiftHashDivcFuzzCase (rng0 : StdGen) : OracleCase × StdGen :=
  ...
  let (case0, rng2) :=
    if shape = 0 then
      ...
      (mkShiftInputCase s!"/fuzz/shape-{shape}/divzero/random" shift #[.num x, .num 0], r3)
    else if shape = 3 then
      let (x, r2) := pickIntFromPool ceilNumeratorPool rng1
      (mkShiftInputCase s!"/fuzz/shape-{shape}/ceil/inexact-pos-den" 1 #[.num x, .num 3], r2)
    else if shape = 4 then
      let (x, r2) := pickIntFromPool ceilNumeratorPool rng1
      (mkShiftInputCase s!"/fuzz/shape-{shape}/ceil/inexact-neg-den" 1 #[.num x, .num (-3)], r2)
      ...
      let (x, r3) := pickIntFromPool ceilNumeratorPool r2
      ...
      (mkShiftInputCase s!"/fuzz/shape-{shape}/ok/boundary/shift256-div-max"
        256 #[.num 1, .num maxInt257], rng1)
...
def suite : InstrSuite where
  id := lshiftHashDivcId
  unit := #[
    { name := "/unit/direct/ceil-rounding-sign-and-inexact-cases"
      run := do
        ...
        for c in checks do
          ...
          let shift := c.2.2.1
          let q := c.2.2.2
          expectOkStack s!"/unit/ceil/x={x}/y={y}/z={shift}"
            (runLshiftHashDivcDirect shift #[intV x, intV y]) #[intV q] }
    ,
    { name := "/unit/direct/boundary-success-and-pop-order"
      run := do
        ...
        expectOkStack "/unit/pop-order/below-null"
          (runLshiftHashDivcDirect 1 #[.null, intV 7, intV 3]) #[.null, intV 5]
        expectOkStack "/unit/pop-order/below-cell"
          (runLshiftHashDivcDirect 2 #[.cell Cell.empty, intV (-7), intV 3])
    ...
    { name := "/unit/error/divzero-overflow-underflow-type-and-order"
      run := do
        expectErr "/unit/error/divzero/nonzero-over-zero"
          (runLshiftHashDivcDirect 1 #[intV 5, intV 0]) .intOv
        expectErr "/unit/error/overflow/max-shift1-div1"
          (runLshiftHashDivcDirect 1 #[intV maxInt257, intV 1]) .intOv
        expectErr "/unit/error/overflow/min-shift1-div1"
          (runLshiftHashDivcDirect 1 #[intV minInt257, intV 1]) .intOv
        expectErr "/unit/error/underflow/empty"
          (runLshiftHashDivcDirect 1 #[]) .stkUnd
        expectErr "/unit/error/underflow/one-item"
          (runLshiftHashDivcDirect 1 #[intV 1]) .stkUnd
        expectErr "/unit/error-order/underflow-before-type-single-non-int"
          (runLshiftHashDivcDirect 1 #[.null]) .stkUnd
        expectErr "/unit/error/type/y-top-non-int"
          (runLshiftHashDivcDirect 1 #[intV 7, .null]) .typeChk
        expectErr "/unit/error/type/x-second-non-int"
          (runLshiftHashDivcDirect 1 #[.null, intV 7]) .typeChk
        expectErr "/unit/error-order/both-non-int-y-first"
          (runLshiftHashDivcDirect 1 #[.null, .cell Cell.empty]) .typeChk }
    ,
    { name := "/unit/error/immediate-range-gate-before-pop"
      run := do
        expectErr "/unit/error/range/shift0-before-y-pop"
          (runLshiftHashDivcDirect 0 #[.null, .cell Cell.empty]) .rangeChk
        expectErr "/unit/error/range/shift257-before-y-pop"
          (runLshiftHashDivcDirect 257 #[.cell Cell.empty, .null]) .rangeChk
        expectErr "/unit/error/underflow/shift0-empty-before-range-gate"
          (runLshiftHashDivcDirect 0 #[]) .stkUnd }
    ,
    { name := "/unit/opcode/decode-hash-sequence"
      run := do
        let instr1 := mkLshiftHashDivcInstr 1
        let instr256 := mkLshiftHashDivcInstr 256
        ...
        let s1 ← expectDecodeStep "decode/lshift-hash-divc-z1" s0 instr1 24
        let s2 ← expectDecodeStep "decode/lshift-hash-divc-z256" s1 instr256 24
    ...
    ,
    { name := "/unit/dispatch/non-lshift-hash-divc-falls-through"
      run := do
        expectOkStack "/unit/dispatch/fallback"
          (runLshiftHashDivcDispatchFallback #[]) #[intV 5517] }
  ]
  oracle := #[
    mkShiftInputCase "/ok/ceil/pos-pos-inexact" 1 #[.num 7, .num 3],
    mkShiftInputCase "/ok/ceil/neg-pos-inexact" 1 #[.num (-7), .num 3],
    mkShiftInputCase "/ok/ceil/pos-neg-inexact" 1 #[.num 7, .num (-3)],
    mkShiftInputCase "/ok/ceil/neg-neg-inexact" 1 #[.num (-7), .num (-3)],
    mkShiftInputCase "/ok/ceil/pos-pos-inexact-small" 1 #[.num 5, .num 3],
    mkShiftInputCase "/ok/ceil/neg-pos-inexact-small" 1 #[.num (-5), .num 3],
    mkShiftInputCase "/ok/ceil/pos-neg-inexact-small" 1 #[.num 5, .num (-3)],
    mkShiftInputCase "/ok/ceil/neg-neg-inexact-small" 1 #[.num (-5), .num (-3)],
    mkShiftInputCase "/ok/ceil/neg-pos-near-zero" 1 #[.num (-1), .num 3],
    mkShiftInputCase "/ok/ceil/pos-neg-near-zero" 1 #[.num 1, .num (-3)],
    mkShiftInputCase "/ok/ceil/neg-neg-near-zero" 1 #[.num (-1), .num (-3)],
    ...
    mkShiftInputCase "/ok/boundary/one-div-max-shift256" 256 #[.num 1, .num maxInt257],
    ...
    mkCase "/gas/exact-cost-succeeds" #[intV 7, intV 3]
      #[.pushInt (.num lshiftHashDivcSetGasExact), .tonEnvOp .setGasLimit, lshiftHashDivcGasProbeInstr],
    mkCase "/gas/exact-minus-one-out-of-gas" #[intV 7, intV 3]
      #[.pushInt (.num lshiftHashDivcSetGasExactMinusOne), .tonEnvOp .setGasLimit, lshiftHashDivcGasProbeInstr]
  ]
  fuzz := #[
    { seed := 2026020857
      count := 700
      gen := genLshiftHashDivcFuzzCase }
  ]
