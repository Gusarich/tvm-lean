...
/-
GEQ branch-mapping notes (Lean + C++ reference):
- Lean analyzed file: `TvmLean/Semantics/Exec/Arith/Geq.lean`
- C++ analyzed file: `/Users/daniil/Coding/ton/crypto/vm/arithops.cpp`
  (`exec_cmp`, opcode registration in `register_int_cmp_ops` with mode `0x778`).
...
- C++: 6 branch points / 9 aligned terminal outcomes
  (`GEQ` registration to `exec_cmp(..., mode=0x778, quiet=false)`;
...
- `intOv` from NaN propagation in non-quiet GEQ;
...
- NaN handling must throw `intOv` in non-quiet GEQ;
- deterministic gas threshold for `PUSHINT n; SETGASLIMIT; GEQ`.
-/

private def geqId : InstrId := { name := "GEQ" }

private def mkCase
    (name : String)
    (stack : Array Value)
    (program : Array Instr := #[.geq])
...
private def runGeqDirect (stack : Array Value) : Except Excno (Array Value) :=
  runHandlerDirect execInstrArithGeq .geq stack

private def runGeqDispatchFallback (stack : Array Value) : Except Excno (Array Value) :=
  runHandlerDirectWithNext execInstrArithGeq .add (VM.push (intV 777)) stack

private def geqSetGasExact : Int :=
  computeExactGasBudget .geq

private def geqSetGasExactMinusOne : Int :=
  computeExactGasBudgetMinusOne .geq

private def genGeqFuzzCase (rng0 : StdGen) : OracleCase × StdGen :=
    ...
    { name := "unit/finite-ordering-sign-and-boundary-cases"
      run := do
        ...
        for c in checks do
          ...
          let y := c.2.1
          let expected := c.2.2
          expectOkStack s!"{x}>={y}" (runGeqDirect #[intV x, intV y]) #[intV expected] }
    ,
    { name := "unit/nan-propagation-throws-intov"
      run := do
        expectErr "x-nan" (runGeqDirect #[.int .nan, intV 1]) .intOv
        expectErr "y-nan" (runGeqDirect #[intV 1, .int .nan]) .intOv
        expectErr "both-nan" (runGeqDirect #[.int .nan, .int .nan]) .intOv }
    ,
    { name := "unit/type-errors-follow-pop-order"
      run := do
        expectErr "pop-y-first-null" (runGeqDirect #[intV 1, .null]) .typeChk
        expectErr "pop-x-second-null" (runGeqDirect #[.null, intV 1]) .typeChk
        expectErr "both-non-int-y-first" (runGeqDirect #[.cell Cell.empty, .null]) .typeChk }
    ,
    { name := "unit/underflow-ordering-on-short-stacks"
      run := do
        expectErr "empty" (runGeqDirect #[]) .stkUnd
        expectErr "one-int" (runGeqDirect #[intV 1]) .stkUnd
        expectErr "one-non-int-underflow-first" (runGeqDirect #[.null]) .stkUnd }
    ,
    { name := "unit/dispatch/non-geq-falls-through"
      run := do
        expectOkStack "dispatch/fallback" (runGeqDispatchFallback #[]) #[intV 777] }
  ]
  oracle := #[
    ...
    mkCase "nan/y-via-pushnan-intov" #[intV 42] #[.pushInt .nan, .geq],
    mkCase "nan/x-via-pushnan-xchg-intov" #[intV 42] #[.pushInt .nan, .xchg0 1, .geq],
    mkCase "nan/both-via-double-pushnan-intov" #[] #[.pushInt .nan, .pushInt .nan, .geq],
    mkCase "gas/exact-cost-succeeds" #[intV 7, intV 5]
      #[.pushInt (.num geqSetGasExact), .tonEnvOp .setGasLimit, .geq],
    mkCase "gas/exact-minus-one-out-of-gas" #[intV 7, intV 5]
      #[.pushInt (.num geqSetGasExactMinusOne), .tonEnvOp .setGasLimit, .geq]
  ]
  fuzz := #[
    { seed := 2026020806
      count := 500
