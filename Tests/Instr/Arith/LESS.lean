...
namespace Tests.Instr.Arith.LESS

/-
LESS branch-mapping notes (Lean + C++ reference):
- Lean analyzed file: `TvmLean/Semantics/Exec/Arith/Less.lean`
- C++ analyzed file: `/Users/daniil/Coding/ton/crypto/vm/arithops.cpp`
  (`exec_cmp`, opcode registration in `register_int_cmp_ops` with mode `0x887`).
...
- C++: 6 branch points / 9 aligned terminal outcomes
  (`LESS` registration to `exec_cmp(..., mode=0x887, quiet=false)`;
   explicit `check_underflow(2)`; two `pop_int` type checks; invalid-int (NaN) path;
   compare result class `{<,=,>}` mapped by mode to `{-1,0,0}`).
...
- `intOv` from NaN propagation in non-quiet LESS;
- out-of-gas at exact-minus-one budget.

Key risk areas:
- truth-table mapping for mode `0x887` (`<` only maps to `-1`);
- operand pop order (`y` then `x`) and corresponding error ordering;
- underflow-vs-type ordering for one-item stacks (`check_underflow(2)`);
- NaN handling must throw `intOv` in non-quiet LESS;
- deterministic gas threshold for `PUSHINT n; SETGASLIMIT; LESS`.
-/

private def lessId : InstrId := { name := "LESS" }

private def mkCase
    (name : String)
    (stack : Array Value)
    (program : Array Instr := #[.less])
...
private def runLessDirect (stack : Array Value) : Except Excno (Array Value) :=
  runHandlerDirect execInstrArithLess .less stack

private def runLessDispatchFallback (stack : Array Value) : Except Excno (Array Value) :=
  runHandlerDirectWithNext execInstrArithLess .add (VM.push (intV 777)) stack

private def lessSetGasExact : Int :=
  computeExactGasBudget .less

private def lessSetGasExactMinusOne : Int :=
  computeExactGasBudgetMinusOne .less

private def genLessFuzzCase (rng0 : StdGen) : OracleCase × StdGen :=
    ...
    { name := "unit/finite-ordering-sign-and-boundary-cases"
      run := do
        ...
        for c in checks do
          ...
          let y := c.2.1
          let expected := c.2.2
          expectOkStack s!"{x}<{y}" (runLessDirect #[intV x, intV y]) #[intV expected] }
    ,
    { name := "unit/nan-propagation-throws-intov"
      run := do
        expectErr "x-nan" (runLessDirect #[.int .nan, intV 1]) .intOv
        expectErr "y-nan" (runLessDirect #[intV 1, .int .nan]) .intOv
        expectErr "both-nan" (runLessDirect #[.int .nan, .int .nan]) .intOv }
    ,
    { name := "unit/type-errors-follow-pop-order"
      run := do
        expectErr "pop-y-first-null" (runLessDirect #[intV 1, .null]) .typeChk
        expectErr "pop-x-second-null" (runLessDirect #[.null, intV 1]) .typeChk
        expectErr "both-non-int-y-first" (runLessDirect #[.cell Cell.empty, .null]) .typeChk }
    ,
    { name := "unit/underflow-ordering-on-short-stacks"
      run := do
        expectErr "empty" (runLessDirect #[]) .stkUnd
        expectErr "one-int" (runLessDirect #[intV 1]) .stkUnd
        expectErr "one-non-int-underflow-first" (runLessDirect #[.null]) .stkUnd }
    ,
    { name := "unit/dispatch/non-less-falls-through"
      run := do
        expectOkStack "dispatch/fallback" (runLessDispatchFallback #[]) #[intV 777] }
  ]
  oracle := #[
    ...
    mkCase "nan/y-via-pushnan-intov" #[intV 42] #[.pushInt .nan, .less],
    mkCase "nan/x-via-pushnan-xchg-intov" #[intV 42] #[.pushInt .nan, .xchg0 1, .less],
    mkCase "nan/both-via-double-pushnan-intov" #[] #[.pushInt .nan, .pushInt .nan, .less],
    mkCase "gas/exact-cost-succeeds" #[intV 7, intV 5]
      #[.pushInt (.num lessSetGasExact), .tonEnvOp .setGasLimit, .less],
    mkCase "gas/exact-minus-one-out-of-gas" #[intV 7, intV 5]
      #[.pushInt (.num lessSetGasExactMinusOne), .tonEnvOp .setGasLimit, .less]
  ]
  fuzz := #[
    { seed := 2026020809
      count := 500
