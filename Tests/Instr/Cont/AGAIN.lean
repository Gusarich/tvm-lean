...
/-!
AGAIN / AGAINBRK branch-mapping notes (Lean + C++ reference):
- Lean analyzed files:
  - `TvmLean/Semantics/Exec/Flow/LoopExt.lean` (`.again brk`)
  - `TvmLean/Semantics/Step/Step.lean` and `.../Runvm.lean` (`.againBody`)
  - `TvmLean/Semantics/Exec/Common.lean` (`extractCc`)
- C++ analyzed files:
  - `/Users/daniil/Coding/ton/crypto/vm/contops.cpp` (`exec_again`)
  - `/Users/daniil/Coding/ton/crypto/vm/continuation.cpp` (`AgainCont::jump/_w`)
  - `/Users/daniil/Coding/ton/crypto/vm/vm.cpp` (`extract_cc`, `c1_save_set`)

Covered branches and error order:
1. Dispatch: `.contExt (.again brk)` handled vs fallback to `next`.
2. AGAIN (`brk=false`):
   - `pop_cont` underflow/type checks;
   - successful pop installs `.againBody body`.
3. AGAINBRK (`brk=true`):
   - `extract_cc(3)` must run before `pop_cont`;
   - `extract_cc(3)` type-checks current `cc` (must be ordinary), saves `{c0,c1}` into captured continuation, resets `c0`, then `set_c1(cc)`;
   - only then `pop_cont` underflow/type checks run.
4. Runtime `.againBody`:
   - `body.hasC0` true: jump to body without rewriting `c0`;
   - `body.hasC0` false: set `c0 := againBody(body)` and jump to body;
   - jump-time `nargs` underflow on body continuations (C++ `adjust_jump_cont`) must throw `stkUnd`.

Mismatch found and fixed:
- Lean AGAINBRK used an ad-hoc `c1 := after` path and popped body before C++-required
  `extract_cc(3)` effects/order.
- Lean `.againBody` missed jump-time `nargs` underflow checks for loop-body continuations.
-/

private def againId : InstrId := { name := "AGAIN" }

private def againInstr : Instr := .contExt (.again false)

private def againBrkInstr : Instr := .contExt (.again true)

private def dispatchSentinel : Int := 73191

private def tailMarker : Int := 606
...
private def againTailPushProgram : Array Instr :=
  #[againInstr, .pushInt (.num tailMarker)]

private def againTailAddProgram : Array Instr :=
  #[againInstr, .add]

private def againTailRetAltProgram : Array Instr :=
  #[againInstr, .retAlt]

private def mkCase
    (name : String)
    (stack : Array Value)
    (program : Array Instr := #[againInstr])
...
private def runAgainDirect (stack : Array Value) : Except Excno (Array Value) :=
  runHandlerDirect execInstrFlowLoopExt againInstr stack

private def runAgainDispatchFallback
    (instr : Instr)
    (stack : Array Value) : Except Excno (Array Value) :=
  runHandlerDirectWithNext execInstrFlowLoopExt instr (VM.push (intV dispatchSentinel)) stack

private def runAgainRaw
    (instr : Instr)
    (stack : Array Value)
...
private def againSetGasExact : Int :=
  computeExactGasBudget againInstr
...
def suite : InstrSuite where
  ...
  unit := #[
    { name := "unit/dispatch/match-vs-fallback"
      run := do
        expectOkStack "dispatch/fallback-add"
          (runAgainDispatchFallback .add #[intV 1])
          #[intV 1, intV dispatchSentinel]
        expectOkStack "dispatch/matched-again"
          (runAgainDispatchFallback againInstr #[kCont])
          #[] }
    ,
    { name := "unit/direct/underflow-type-and-pop-order"
      run := do
        expectErr "underflow/empty" (runAgainDirect #[]) .stkUnd
        expectErr "type/top-int" (runAgainDirect #[intV 1]) .typeChk
        expectErr "type/top-null" (runAgainDirect #[.null]) .typeChk
        expectErr "type/top-cell" (runAgainDirect #[.cell refCellA]) .typeChk
        expectErr "type/top-slice" (runAgainDirect #[.slice sliceA]) .typeChk
        expectErr "type/top-builder" (runAgainDirect #[.builder Builder.empty]) .typeChk
        expectErr "type/top-tuple-empty" (runAgainDirect #[.tuple #[]]) .typeChk
        expectErr "order/top-before-below-cont" (runAgainDirect #[kCont, intV 9]) .typeChk

        let stType ← expectRawErr "order/top-pop-before-type"
          (runAgainRaw againInstr #[intV 77, .null]) .typeChk
        if stType.stack != #[intV 77] then
          throw (IO.userError s!"order/top-pop-before-type: expected stack #[77], got {reprStr stType.stack}") }
    ,
    { name := "unit/state/success-installs-againBody"
      run := do
        let oldC0 : Continuation := .quit 41
        let oldC1 : Continuation := .quit 42
        let st ← expectRawOk "state/success"
          (runAgainRaw againInstr #[intV 44, .cont bodyNoC0] defaultOrdCc oldC0 oldC1)
        if st.stack != #[intV 44] then
          throw (IO.userError s!"state/success: stack mismatch {reprStr st.stack}")
        else if st.cc != .againBody bodyNoC0 then
          throw (IO.userError s!"state/success: expected cc=againBody, got {reprStr st.cc}")
        else if st.regs.c0 != oldC0 then
          throw (IO.userError s!"state/success: expected c0 unchanged {reprStr oldC0}, got {reprStr st.regs.c0}")
        else if st.regs.c1 != oldC1 then
          throw (IO.userError s!"state/success: expected c1 unchanged {reprStr oldC1}, got {reprStr st.regs.c1}")
        else
          pure () }
    ,
    { name := "unit/runtime/againBody-branches"
      run := do
        ...
        let stNoC00 : VmState :=
          { base with
            regs := { base.regs with c0 := .quit 51 }
        ...
        let stHasC00 : VmState :=
          { base with
            regs := { base.regs with c0 := .quit 52 }
        ...
        else if stHasC0.regs.c0 != .quit 52 then
          throw (IO.userError s!"runtime/has-c0: expected c0 unchanged, got {reprStr stHasC0.regs.c0}")
        else
          pure () }
    ,
    { name := "unit/runtime/againBody-nargs-underflow"
      run := do
        let st0 : VmState :=
          { (VmState.initial Cell.empty) with
            stack := #[intV 1]
            cc := .againBody bodyNeedsTwoArgs }
        let st ← expectContinue "runtime/nargs-underflow" (stepOnce st0)
        if st.cc != st0.regs.c2 then
          throw (IO.userError s!"runtime/nargs-underflow: expected cc=c2, got {reprStr st.cc}")
        else if st.stack != #[intV 0, intV Excno.stkUnd.toInt] then
          throw (IO.userError
            s!"runtime/nargs-underflow: expected stack #[0,{Excno.stkUnd.toInt}], got {reprStr st.stack}")
        else
          pure () }
    ,
    { name := "unit/brk/success-uses-extractcc3-before-popcont"
      run := do
        ...
        let st ← expectRawOk "brk/success"
          (runAgainRaw againBrkInstr #[intV 5, .cont bodyNoC0] cc0 oldC0 oldC1)
    ...
    ,
    { name := "unit/brk/order/nonordinary-cc-before-popcont"
      run := do
        let oldC0 : Continuation := .quit 71
        let oldC1 : Continuation := .quit 72
        let st ← expectRawErr "brk/nonordinary-cc"
          (runAgainRaw againBrkInstr #[intV 11, .null] (.quit 7) oldC0 oldC1) .typeChk
        if st.stack != #[intV 11, .null] then
          ...
          pure () }
    ,
    { name := "unit/brk/order/extract-before-popcont-errors"
      run := do
        ...
        let stType ← expectRawErr "brk/order/type-after-extract"
          (runAgainRaw againBrkInstr #[intV 13, .null] cc0 oldC0 oldC1) .typeChk
        ...
        let stUnd ← expectRawErr "brk/order/underflow-after-extract"
          (runAgainRaw againBrkInstr #[] cc0 oldC0 oldC1) .stkUnd
        if stUnd.stack != #[] then
  ...
  oracle := #[
    -- Direct AGAIN with oracle-encodable continuation token K=quit(0).
    ...
    -- Tail instructions should be skipped after AGAIN transfers control.
    mkCase "ok/tail-push-skipped/basic" #[kCont] againTailPushProgram,
    mkCase "ok/tail-push-skipped/int-prefix" #[intV 4, kCont] againTailPushProgram,
    mkCase "ok/tail-push-skipped/mixed-prefix" #[.null, .cell refCellB, kCont] againTailPushProgram,
    mkCase "ok/tail-push-skipped/max-int257" #[intV maxInt257, kCont] againTailPushProgram,
    mkCase "ok/tail-push-skipped/deep-long" (noiseLong ++ #[kCont]) againTailPushProgram,
    mkCase "ok/tail-add-skipped/two-ints" #[intV 2, intV 3, kCont] againTailAddProgram,
    mkCase "ok/tail-add-skipped/with-noise" #[.null, intV 10, intV (-6), .cell refCellA, kCont] againTailAddProgram,
    mkCase "ok/tail-add-skipped/min-max" #[intV minInt257, intV maxInt257, kCont] againTailAddProgram,
    mkCase "ok/tail-retalt-skipped/basic" #[kCont] againTailRetAltProgram,
    mkCase "ok/tail-retalt-skipped/deep" #[.builder Builder.empty, .tuple #[], intV 7, kCont] againTailRetAltProgram,
    mkCase "ok/tail-retalt-skipped/mixed" #[.slice sliceA, .null, intV 15, kCont] againTailRetAltProgram,
    mkCase "ok/tail-push-skipped/noise-a" (noiseA ++ #[kCont]) againTailPushProgram,
    mkCase "ok/tail-push-skipped/noise-b" (noiseB ++ #[kCont]) againTailPushProgram,
    mkCase "ok/tail-add-skipped/noise-long" (noiseLong ++ #[intV 8, intV 9, kCont]) againTailAddProgram,
    mkCase "ok/tail-retalt-skipped/noise-a" (noiseA ++ #[kCont]) againTailRetAltProgram,
    ...
    mkCase "gas/exact-succeeds" #[kCont]
      #[.pushInt (.num againSetGasExact), .tonEnvOp .setGasLimit, againInstr],
