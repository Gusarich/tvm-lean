...
5. [B5] By-ref value shape.
   - `DICTIREMMAXREF` requires value representation `bits=0, refs=1`;
     otherwise `.dictErr`.

6. [B6] Signed key reconstruction.
   - `unsigned=false`, `intKey=true` uses signed 2s-complement decoding.

7. [B7] Assembler behavior.
   - `.dictGetMinMax 29` assembles to `0xF49D`.
   - In-range gap/bad encoding combinations reject with `.invOpcode`/`.rangeChk`.

8. [B8] Decoder behavior.
   - `0xF49D` decodes to `.dictGetMinMax 29`.
...
private def dictSingleRef8Neg : Cell :=
  mkDictSetRefRoot! "dictSingleRef8Neg" 8 #[( -1, valueB)]
private def dictTwoRef8 : Cell :=
  mkDictSetRefRoot! "dictTwoRef8" 8 #[( -1, valueB), (5, valueA)]
private def dictTwoRef8AfterMax : Cell :=
  mkDictSetRefRoot! "dictTwoRef8AfterMax" 8 #[( -1, valueB)]
private def dictThreeRef8 : Cell :=
  mkDictSetRefRoot! "dictThreeRef8" 8 #[( -128, valueA), (0, valueB), (5, valueC)]
...
def genDictIremMaxRefFuzzCase (rng0 : StdGen) : OracleCase × StdGen :=
  ...
  let (case0, rng2) :=
    if shape = 0 then
      ...
      (mkCase "fuzz/ok/hit/single8-alt" #[ .cell dictSingleRef8, intV 5], rng1)
    else if shape = 5 then
      (mkCase "fuzz/ok/hit/single8-neg" #[ .cell dictSingleRef8Neg, intV 8], rng1)
...
def suite : InstrSuite where
  ...
  unit := #[
    ...
    { name := "unit/asm/assemble-ok" -- [B7]
      run := do
        match assembleCp0 [instr] with
        | .ok c =>
            if c.bits = natToBits 0xF49D 16 then
              pure ()
            else
              throw (IO.userError s!"expected 0xF49D bits, got {reprStr c.bits}")
        | .error e =>
            throw (IO.userError s!"assemble DICTIREMMAXREF failed: {reprStr e}") },
    ...
    { name := "unit/decode/self-and-neighbors" -- [B8]
      run := do
        expectDecodeOk "decode/self" rawOpcodeF49D (.dictGetMinMax 29)
        expectDecodeOk "decode/prev" rawOpcodeF49C (.dictGetMinMax 28)
        expectDecodeOk "decode/next" rawOpcodeF49E (.dictGetMinMax 30)
        expectDecodeOk "decode/nextnext" rawOpcodeF49F (.dictGetMinMax 31) },
    ...
    { name := "unit/exec/hit-single8-neg" -- [B6]
      run := do
        expectOkStack "hit-single8-neg"
          (runDirect #[ .cell dictSingleRef8Neg, intV 8])
          #[.null, .cell valueB, intV (-1), intV (-1)] },
  ...
  oracle := #[
    ...
    mkCase "oracle/hit/single8-alt-5" #[.cell dictSingleRef8, intV 5],
    mkCase "oracle/hit/single8-neg" #[.cell dictSingleRef8Neg, intV 8],
