...
namespace Tests.Instr.Dict.DICTUREPLACEGET

/--!
INSTRUCTION: DICTUREPLACEGET

BRANCH ANALYSIS (derived from Lean + C++ source):

1. [B1] Dispatch selection.
   - `execInstrDictExt` handles `.dictExt (.mutGet ... )`.
   - This instruction is only `DICTUREPLACEGET` variants (slice/int key, signed/unsigned, ref/non-ref);
...
private def runDICTUREPLACEGETDirect (instr : Instr) (stack : Array Value) : Except Excno (Array Value) :=
  runHandlerDirect execInstrDictExt instr stack

def genDICTUREPLACEGETFuzzCase (rng0 : StdGen) : OracleCase × StdGen :=
...
def suite : InstrSuite where
  ...
  unit := #[
    { name := "unit/dispatch/fallback" -- [B1]
      run := do
      match runDICTUREPLACEGETFallback #[] with
      | .ok st =>
    ...
    { name := "unit/runtime/underflow-empty" -- [B2]
      run := do
        expectErr "underflow" (runDICTUREPLACEGETDirect instrSlice #[]) .stkUnd },
    { name := "unit/runtime/underflow-three" -- [B2]
      run := do
        expectErr "underflow-three" (runDICTUREPLACEGETDirect instrSlice #[.slice keySlice5_8, .null, intV 8]) .stkUnd },
    { name := "unit/runtime/n-negative" -- [B3]
      run := do
        expectErr "n-negative" (runDICTUREPLACEGETDirect instrSlice (mkSliceCaseStack (.slice valueSliceA) keySlice5_8 (.cell dictSliceSingle8) (-1))) .rangeChk },
    { name := "unit/runtime/n-too-large" -- [B3]
      run := do
        expectErr "n-too-large" (runDICTUREPLACEGETDirect instrSlice (mkSliceCaseStack (.slice valueSliceA) keySlice5_8 (.cell dictSliceSingle8) 1024)) .rangeChk },
    { name := "unit/runtime/n-nan" -- [B3]
      run := do
        expectErr "n-nan" (runDICTUREPLACEGETDirect instrSlice (#[(.slice valueSliceA), .slice keySlice5_8, .cell dictSliceSingle8, .int .nan])) .rangeChk },
    { name := "unit/runtime/key-short" -- [B3]
      run := do
        expectErr "key-short" (runDICTUREPLACEGETDirect instrSlice (mkSliceCaseStack (.slice valueSliceA) keySliceShort4 (.cell dictSliceSingle8) 8)) .cellUnd },
    { name := "unit/runtime/type-root" -- [B4]
      run := do
        expectErr "type-root" (runDICTUREPLACEGETDirect instrSlice (mkSliceCaseStack (.slice valueSliceA) keySlice5_8 (.int (.num 0)) 8)) .typeChk },
    { name := "unit/runtime/type-key" -- [B4]
      run := do
        expectErr "type-key" (runDICTUREPLACEGETDirect instrSlice (#[(.slice valueSliceA), .null, (.cell dictSliceSingle8), intV 8])) .typeChk },
    ...
    { name := "unit/runtime/miss-slice-null" -- [B4]
      run := do
        expectOkStack "miss-slice-null"
          (runDICTUREPLACEGETDirect instrSlice (mkSliceCaseStack (.slice valueSliceA) keySlice5_8 .null 8))
          #[.null, intV 0] },
    ...
    { name := "unit/runtime/miss-int" -- [B4]
      run := do
        expectOkStack "miss-int"
          (runDICTUREPLACEGETDirect instrInt (mkIntCaseStack (.slice valueSliceA) 2 (.cell dictIntSigned8Single) 8))
          #[.cell dictIntSigned8Single, intV 0] },
    { name := "unit/runtime/hit-byref" -- [B5]
      run := do
        expectOkStack "hit-byref"
          (runDICTUREPLACEGETDirect instrIntRef (mkIntCaseStack (.cell valueCellB) 5 (.cell dictIntRefSigned8Single) 8))
          #[.cell dictIntRefSigned8SingleRepl, .cell valueCellA, intV (-1)] },
    { name := "unit/runtime/miss-byref" -- [B4]
      run := do
        expectOkStack "miss-byref"
          (runDICTUREPLACEGETDirect instrIntRef (mkIntCaseStack (.cell valueCellA) 5 .null 8))
          #[.null, intV 0] },
    { name := "unit/runtime/byref-bad-shape" -- [B5]
      run := do
        expectErr "byref-bad-shape" (runDICTUREPLACEGETDirect instrIntRef (mkIntCaseStack (.cell valueCellB) 5 (.cell dictIntBadByRefBits) 8)) .dictErr },
    ...
    { name := "unit/gas/base-exact-minus-one" -- [B9]
      run := do
        ...
        match runDICTUREPLACEGETDirect instrSlice (mkSliceCaseStack (.slice valueSliceA) keySlice5_8 .null 8) with
        | .ok _ => pure ()
        | .error _ => pure () }]
  oracle := #[
  ...
  fuzz := #[
    { seed := fuzzSeedForInstr suiteId
      count := 500
      gen := genDICTUREPLACEGETFuzzCase }
  ]

initialize registerSuite suite

end Tests.Instr.Dict.DICTUREPLACEGET
