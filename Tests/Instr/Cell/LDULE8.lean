...
/-!
LDULE8 branch-mapping notes (Lean + C++ reference):
- Lean analyzed files:
  - `TvmLean/Semantics/Exec/CellOp/LoadLeInt.lean` (`.loadLeInt true 8 false false`)
...
- C++ authoritative file:
  - `/Users/daniil/Coding/ton/crypto/vm/cellops.cpp` (`exec_load_le_int`, args=3 for LDULE8)
...
- success path: little-endian 8-byte unsigned decode and non-prefetch stack contract
  (`LD*` pushes `[int, advanced-slice]`);
- opcode family decode around `0xd753`, assembler byte-range guard (`bytes ∈ {4,8}`),
...
private def ldule8Id : InstrId := { name := "LDULE8" }

private def ldule8Instr : Instr :=
  .cellOp (.loadLeInt true 8 false false)

private def ldule8Word : Nat := 0xd753
...
private def mkLdule8Case
    (name : String)
    (stack : Array Value)
    (program : Array Instr := #[ldule8Instr])
...
private def runLdule8Direct (stack : Array Value) : Except Excno (Array Value) :=
  runHandlerDirect execInstrCellOpLoadLeIntOnly ldule8Instr stack

private def runLdule8DispatchFallback (instr : Instr) (stack : Array Value) :
...
private def expectedSuccessStack (below : Array Value) (s : Slice) : Array Value :=
  below ++ #[intV (expectedUnsignedFromSlice s), .slice (s.advanceBits 64)]
  ...
  unit := #[
      ...
      run := do
        ...
        for (label, bytes, tail) in checks do
          ...
          expectOkStack s!"ok/{label}/tail-{tail.size}"
            (runLdule8Direct #[.slice input])
            #[intV (expectedUnsignedFromBytes bytes), .slice (input.advanceBits 64)]
        ...
        expectOkStack "ok/refs-present-keep-tail-and-refs-on-remainder"
          (runLdule8Direct #[.slice refsInput])
        ...
        expectOkStack "ok/deep-stack-preserved"
          (runLdule8Direct (below ++ #[.slice deepInput]))
          (expectedSuccessStack below deepInput)

        expectOkStack "ok/partial-slice-offset"
          (runLdule8Direct #[.slice partialCursorSlice])
      ...
      run := do
        ...
        for avail in insuff do
          ...
          expectErr s!"cellund/avail-{avail}"
            (runLdule8Direct #[.slice s]) .cellUnd

        expectErr "cellund/refs-only-no-bits"
          (runLdule8Direct #[.slice (mkSliceWithBitsRefs #[] #[refLeafA, refLeafB])]) .cellUnd

        expectErr "cellund/short-cursor-avail59"
          (runLdule8Direct #[.slice shortCursorSlice]) .cellUnd }
    ,
    { name := "unit/direct/popSlice-underflow-type-and-ordering"
      run := do
        expectErr "underflow/empty" (runLdule8Direct #[]) .stkUnd
        expectErr "type/top-null" (runLdule8Direct #[.null]) .typeChk
        expectErr "type/top-int" (runLdule8Direct #[intV 9]) .typeChk
        expectErr "type/top-cell" (runLdule8Direct #[.cell Cell.empty]) .typeChk
        expectErr "type/top-builder" (runLdule8Direct #[.builder Builder.empty]) .typeChk
        expectErr "type/deep-top-not-slice"
          (runLdule8Direct #[.slice (mkSliceFromLeBytes bytesOne), .null]) .typeChk
        expectErr "type/order-top-not-slice-over-short-slice"
          (runLdule8Direct #[.slice (mkSliceWithBitsRefs (stripeBits 8 0)), intV 3]) .typeChk
        expectErr "cellund/order-short-slice-over-invalid-below"
          (runLdule8Direct #[.null, .slice (mkSliceWithBitsRefs (stripeBits 8 1))]) .cellUnd }
    ,
    { name := "unit/direct/rangechk-unreachable-for-runtime-path"
      run := do
        let probes : Array (String × Except Excno (Array Value)) :=
          #[
            ("success", runLdule8Direct #[.slice (mkSliceFromLeBytes bytesOne)]),
            ("cellund", runLdule8Direct #[.slice (mkSliceWithBitsRefs (stripeBits 31 0))]),
            ("underflow", runLdule8Direct #[]),
            ("type", runLdule8Direct #[.null])
          ]
        for p in probes do
          match p.2 with
          | .error .rangeChk =>
              throw (IO.userError s!"{p.1}: unexpected rangeChk for LDULE8 runtime path")
          | _ => pure () }
    ,
    { name := "unit/opcode/decode-family-and-assembler-boundaries"
      run := do
        ...
        let s5 ← expectDecodeStep "decode/raw-ldule8" s4 ldule8Instr 16
        let s6 ← expectDecodeStep "decode/raw-neighbor-ldule8q" s5 (.cellOp (.loadLeInt true 8 false true)) 16
        let s7 ← expectDecodeStep "decode/raw-neighbor-pldule8" s6 (.cellOp (.loadLeInt true 8 true false)) 16
        ...
        let singleCode ←
          match assembleCp0 [ldule8Instr] with
          | .ok cell => pure cell
          | .error e => throw (IO.userError s!"assemble/ldule8-single failed: {e}")
        if singleCode.bits = natToBits ldule8Word 16 then
          pure ()
        else
          throw (IO.userError s!"opcode/ldule8: expected 0xd753 encoding, got bits {singleCode.bits}")

        let asmCode ←
          match assembleCp0 [ldule8Instr, .add] with
          | .ok cell => pure cell
          | .error e => throw (IO.userError s!"assemble/ldule8 failed: {e}")
        let a0 := Slice.ofCell asmCode
        let a1 ← expectDecodeStep "decode/asm-ldule8" a0 ldule8Instr 16
        ...
        match assembleCp0 [.cellOp (.loadLeInt true 5 false false)] with
        | .error .rangeChk => pure ()
        ...
        match assembleCp0 [.cellOp (.loadLeInt true 0 false false)] with
        | .error .rangeChk => pure ()
    ...
    { name := "unit/dispatch/non-loadleint-falls-through"
      run := do
        expectOkStack "dispatch/non-cellop"
          (runLdule8DispatchFallback .add #[.null])
          #[.null, intV dispatchSentinel]
        expectOkStack "dispatch/other-cellop-ldzeroes"
          (runLdule8DispatchFallback (.cellOp .ldZeroes) #[.slice (mkSliceFromLeBytes bytesOne tailBits3)])
          #[.slice (mkSliceFromLeBytes bytesOne tailBits3), intV dispatchSentinel]
        expectOkStack "dispatch/other-cellop-pldrefvar"
          (runLdule8DispatchFallback (.cellOp .pldRefVar) #[intV (-11)])
          #[intV (-11), intV dispatchSentinel] }
  ]
  oracle := #[
    mkLdule8Case "ok/zero/exact" #[.slice (mkSliceFromLeBytes bytesZero)],
    mkLdule8Case "ok/one/exact" #[.slice (mkSliceFromLeBytes bytesOne)],
    mkLdule8Case "ok/max-u64/exact" #[.slice (mkSliceFromLeBytes bytesMaxU64)],
    mkLdule8Case "ok/high-bit/exact" #[.slice (mkSliceFromLeBytes bytesHighBit)],
    mkLdule8Case "ok/max-signed-pos/exact" #[.slice (mkSliceFromLeBytes bytesMaxSignedPos)],
    mkLdule8Case "ok/near-high-bit-plus1/exact" #[.slice (mkSliceFromLeBytes bytesNearHighBitPlusOne)],
    mkLdule8Case "ok/random-0123456789abcdef/exact" #[.slice (mkSliceFromLeBytes bytes0123456789ABCDEF)],
    mkLdule8Case "ok/random-fedcba9876543210/exact" #[.slice (mkSliceFromLeBytes bytesFEDCBA9876543210)],
    mkLdule8Case "ok/alt-lo/exact" #[.slice (mkSliceFromLeBytes bytesAltLo)],
    mkLdule8Case "ok/alt-hi/exact" #[.slice (mkSliceFromLeBytes bytesAltHi)],
    mkLdule8Case "ok/tail3" #[.slice (mkSliceFromLeBytes bytes0123456789ABCDEF tailBits3)],
    mkLdule8Case "ok/tail7" #[.slice (mkSliceFromLeBytes bytesFEDCBA9876543210 tailBits7)],
    mkLdule8Case "ok/tail11" #[.slice (mkSliceFromLeBytes bytesAltHi tailBits11)],
    mkLdule8Case "ok/tail13" #[.slice (mkSliceFromLeBytes bytesAltLo tailBits13)],
    mkLdule8Case "ok/tail23" #[.slice (mkSliceFromLeBytes bytesNearHighBitPlusOne tailBits23)],
    mkLdule8Case "ok/refs1" #[.slice (mkSliceFromLeBytes bytesOne tailBits5 #[refLeafA])],
    mkLdule8Case "ok/refs2-tail7" #[.slice (mkSliceFromLeBytes bytesHighBit tailBits7 #[refLeafA, refLeafB])],
    mkLdule8Case "ok/deep-stack-null-below" #[.null, .slice (mkSliceFromLeBytes bytesMaxU64 tailBits3)],
    mkLdule8Case "ok/deep-stack-two-below" #[intV (-5), .null, .slice (mkSliceFromLeBytes bytesFEDCBA9876543210 tailBits11)],
    mkLdule8Case "ok/deep-stack-cell-below" #[.cell refLeafA, .slice (mkSliceFromLeBytes bytesAltHi tailBits5)],

    mkLdule8Case "cellund/avail0" #[.slice (mkSliceWithBitsRefs #[])],
    mkLdule8Case "cellund/avail1" #[.slice (mkSliceWithBitsRefs (stripeBits 1 0))],
    mkLdule8Case "cellund/avail7" #[.slice (mkSliceWithBitsRefs (stripeBits 7 1))],
    mkLdule8Case "cellund/avail16" #[.slice (mkSliceWithBitsRefs (stripeBits 16 0))],
    mkLdule8Case "cellund/avail31" #[.slice (mkSliceWithBitsRefs (stripeBits 31 1))],
    mkLdule8Case "cellund/avail48" #[.slice (mkSliceWithBitsRefs (stripeBits 48 0))],
    mkLdule8Case "cellund/avail63" #[.slice (mkSliceWithBitsRefs (stripeBits 63 1))],
    mkLdule8Case "cellund/refs-only" #[.slice (mkSliceWithBitsRefs #[] #[refLeafA, refLeafB])],
    mkLdule8Case "cellund/refs-short20" #[.slice (mkSliceWithBitsRefs (stripeBits 20 0) #[refLeafA])],
    mkLdule8Case "cellund/order-short-slice-over-invalid-below"
      #[.null, .slice (mkSliceWithBitsRefs (stripeBits 8 1))],

    mkLdule8Case "underflow/empty" #[],
    mkLdule8Case "type/top-null" #[.null],
    mkLdule8Case "type/top-int" #[intV 0],
    mkLdule8Case "type/top-cell" #[.cell Cell.empty],
    mkLdule8Case "type/top-builder" #[.builder Builder.empty],
    mkLdule8Case "type/deep-top-not-slice" #[.slice (mkSliceFromLeBytes bytesOne), .null],
    mkLdule8Case "type/order-top-not-slice-over-short-slice"
      #[.slice (mkSliceWithBitsRefs (stripeBits 8 0)), intV 3]
  ]
  fuzz := #[
    { seed := 2026021167
      count := 500
      gen := genLdule8FuzzCase }
  ]
