...
/-!
PLDIX branch-mapping notes (Lean + C++ reference):
- Lean analyzed files:
  - `TvmLean/Semantics/Exec/Cell/LoadIntVar.lean`
    (`.loadIntVar unsigned=false prefetch=true quiet=false`)
  - `TvmLean/Model/Instr/Asm/Cp0.lean`
    (`.loadIntVar` encode: `0xd700 + args3`, so PLDIX = `0xd702`)
  - `TvmLean/Model/Instr/Codepage/Cp0.lean`
    (`0xd700..0xd707` decode to `.loadIntVar unsigned prefetch quiet`)
- C++ authority:
  - `/Users/daniil/Coding/ton/crypto/vm/cellops.cpp`
    (`exec_load_int_var`, `exec_load_int_common`, mode=`2` for PLDIX).

Branch map covered by this suite:
- `checkUnderflow 2` before any pop;
- width pop via `popNatUpTo 257` (type/range from `popInt`, including `>257`, negative, NaN);
- slice pop type check only after width checks succeed;
- success contract in prefetch mode: push loaded signed int only (no remainder);
- insufficient bits path throws `cellUnd` in non-quiet mode;
- opcode decode/dispatch checks for PLDIX family and boundaries.
-/

private def pldixId : InstrId := { name := "PLDIX" }

private def pldixInstr : Instr := .loadIntVar false true false

private def pldixOpcode : Nat := 0xd702

private def dispatchSentinel : Int := 719

private def mkPldixCase
    (name : String)
    (stack : Array Value)
    (program : Array Instr := #[pldixInstr])
...
private def mkPldixProgramCase
    (name : String)
    (stack : Array Value)
    (program : Array Instr)
    (gasLimits : OracleGasLimits := {})
    (fuel : Nat := 1_000_000) : OracleCase :=
  mkPldixCase name stack program gasLimits fuel

private def runPldixDirect (stack : Array Value) : Except Excno (Array Value) :=
  runHandlerDirect execInstrCellLoadIntVar pldixInstr stack

private def runPldixDispatchFallback (instr : Instr) (stack : Array Value) :
    Except Excno (Array Value) :=
  runHandlerDirectWithNext execInstrCellLoadIntVar instr (VM.push (intV dispatchSentinel)) stack

private def pldixSetGasExact : Int :=
  computeExactGasBudget pldixInstr
...
private def expectedSuccessStack
  ...
  below ++ #[intV (expectedSigned source width)]
...
private def genPldixFuzzCase (rng0 : StdGen) : OracleCase × StdGen :=
  ...
  if shape = 0 then
    ...
    (mkPldixCase s!"fuzz/ok/refs/width-{width}/refs-{refs.size}" #[.slice slice, intV width], rng5)
    ...
    (mkPldixCase s!"fuzz/ok/deep-noise/width-{width}" #[noise, .slice slice, intV width], rng5)
  else if shape = 5 then
    let (availableBits, rng2) := randNat rng1 0 256
    let width := availableBits + 1
    let (slice, rng3) := mkRandomSlice availableBits #[] rng2
    (mkPldixCase s!"fuzz/cellund/short-by-one/bits-{availableBits}" #[.slice slice, intV width], rng3)
  else if shape = 6 then
    let (width, rng2) := randNat rng1 1 257
    (mkPldixCase s!"fuzz/cellund/empty-slice/width-{width}" #[.slice (mkSliceFromBits #[]), intV width], rng2)
    ...
    (mkPldixCase s!"fuzz/cellund/random-short/avail-{availableBits}/width-{width}" #[.slice slice, intV width], rng4)
  else if shape = 8 then
    let (width, rng2) := pickWidthMixed rng1
    let (slice, rng3) := mkRandomSlice width #[] rng2
    (mkPldixCase s!"fuzz/range/negative-width/width-{width}" #[.slice slice, intV (-1)], rng3)
  else if shape = 9 then
    let (width, rng2) := pickWidthMixed rng1
    let (slice, rng3) := mkRandomSlice width #[] rng2
    let (delta, rng4) := randNat rng3 1 2048
    let tooLargeWidth : Int := Int.ofNat (257 + delta)
    (mkPldixCase s!"fuzz/range/too-large-width-{tooLargeWidth}" #[.slice slice, intV tooLargeWidth], rng4)
  else if shape = 10 then
    let (width, rng2) := randNat rng1 0 129
    let (slice, rng3) := mkRandomSlice width #[] rng2
    (mkPldixProgramCase "fuzz/range/width-nan-via-program"
      #[.slice slice]
      #[.pushInt .nan, pldixInstr], rng3)
  else if shape = 11 then
    (mkPldixCase "fuzz/underflow/empty" #[], rng1)
  else if shape = 12 then
    let (pick, rng2) := randNat rng1 0 1
    if pick = 0 then
      (mkPldixCase "fuzz/underflow/one-item-slice" #[.slice (mkSliceFromBits #[])], rng2)
    else
      (mkPldixCase "fuzz/underflow/one-item-width" #[intV 0], rng2)
    ...
    (mkPldixCase "fuzz/type/width-not-int" #[.slice slice, badWidth], rng4)
    ...
    (mkPldixCase s!"fuzz/type/slice-not-slice/width-{width}" #[badSlice, intV width], rng3)
  else if shape = 15 then
    let (badPick, rng2) := randNat rng1 0 2
    let badSlice : Value :=
      if badPick = 0 then .null else if badPick = 1 then .cell refLeafA else .builder Builder.empty
    (mkPldixCase "fuzz/error-order/range-before-slice-type" #[badSlice, intV 4096], rng2)
    ...
    (mkPldixCase "fuzz/error-order/width-type-before-slice-type" #[badSlice, badWidth], rng3)
  else if shape = 17 then
    (mkPldixCase "fuzz/gas/exact"
      #[.slice (mkSliceFromBits (mkPatternBits 8 ++ tailBits11)), intV 8]
      #[.pushInt (.num pldixSetGasExact), .tonEnvOp .setGasLimit, pldixInstr], rng1)
  else if shape = 18 then
    (mkPldixCase "fuzz/gas/exact-minus-one"
      #[.slice (mkSliceFromBits (mkPatternBits 8 ++ tailBits11)), intV 8]
      #[.pushInt (.num pldixSetGasExactMinusOne), .tonEnvOp .setGasLimit, pldixInstr], rng1)
  else
    ...
    (mkPldixCase s!"fuzz/ok/signed257-edge-{pick}" #[.slice edgeSlice, intV 257], rng2)
  ...
  unit := #[
    { name := "unit/direct/prefetch-success-pushes-signed-int-only"
      run := do
        ...
        expectOkStack "ok/deep-stack-order-preserved"
          (runPldixDirect (deepPrefix ++ #[.slice deepSource, intV 5]))
          (expectedSuccessStack deepPrefix deepSource 5)

        expectOkStack "ok/partial-slice-cursor"
          (runPldixDirect #[.slice partialSlice, intV 12])
          (expectedSuccessStack #[] partialSlice 12) }
    ,
    { name := "unit/direct/cellund-when-width-exceeds-bits"
      run := do
        expectErr "cellund/empty-width1"
          (runPldixDirect #[.slice (mkSliceFromBits #[]), intV 1]) .cellUnd
        expectErr "cellund/short-4-vs5"
          (runPldixDirect #[.slice (mkPatternSlice 4), intV 5]) .cellUnd
        expectErr "cellund/short-255-vs256"
          (runPldixDirect #[.slice (mkPatternSlice 255), intV 256]) .cellUnd
        expectErr "cellund/short-256-vs257"
          (runPldixDirect #[.slice (mkPatternSlice 256), intV 257]) .cellUnd
        let refsShort := mkPatternSliceWithRefs 6 #[refLeafA, refLeafB]
        expectErr "cellund/refs-short-6-vs7"
          (runPldixDirect #[.slice refsShort, intV 7]) .cellUnd
        expectErr "cellund/partial-slice-short"
          (runPldixDirect #[.slice shortPartialSlice, intV 10]) .cellUnd }
    ,
    { name := "unit/direct/underflow-type-range-and-error-order"
      run := do
        expectErr "underflow/empty" (runPldixDirect #[]) .stkUnd
        expectErr "underflow/one-item-slice"
          (runPldixDirect #[.slice (mkPatternSlice 3)]) .stkUnd
        expectErr "underflow/one-item-width"
          (runPldixDirect #[intV 7]) .stkUnd
        expectErr "underflow/one-item-null"
          (runPldixDirect #[.null]) .stkUnd

        expectErr "type/width-not-int-null"
          (runPldixDirect #[.slice (mkPatternSlice 3), .null]) .typeChk
        expectErr "type/width-not-int-cell"
          (runPldixDirect #[.slice (mkPatternSlice 3), .cell refLeafA]) .typeChk
        expectErr "type/slice-not-slice-after-valid-width"
          (runPldixDirect #[.null, intV 0]) .typeChk
        expectErr "type/slice-not-slice-cell-after-valid-width"
          (runPldixDirect #[.cell refLeafB, intV 1]) .typeChk
        expectErr "type/slice-not-slice-after-width257"
          (runPldixDirect #[.builder Builder.empty, intV 257]) .typeChk

        expectErr "range/width-negative"
          (runPldixDirect #[.slice (mkPatternSlice 3), intV (-1)]) .rangeChk
        expectErr "range/width-too-large-258"
          (runPldixDirect #[.slice (mkPatternSlice 3), intV 258]) .rangeChk
        expectErr "range/width-too-large-4096"
          (runPldixDirect #[.slice (mkPatternSlice 3), intV 4096]) .rangeChk
        expectErr "range/width-nan"
          (runPldixDirect #[.slice (mkPatternSlice 3), .int .nan]) .rangeChk

        expectErr "error-order/range-before-slice-type"
          (runPldixDirect #[.null, intV 4096]) .rangeChk
        expectErr "error-order/width-type-before-slice-type"
          (runPldixDirect #[.null, .null]) .typeChk }
    ,
    { name := "unit/opcode/decode-and-assembler-boundaries"
      run := do
        ...
        let s3 ← expectDecodeStep "decode/pldix" s2 pldixInstr 16
        ...
        let singleCode ←
          match assembleCp0 [pldixInstr] with
          | .ok cell => pure cell
          | .error e => throw (IO.userError s!"assemble single failed: {e}")
        if singleCode.bits = natToBits pldixOpcode 16 then
          pure ()
        else
          throw (IO.userError s!"opcode/pldix: expected 0xd702 16-bit encoding, got bit-length {singleCode.bits.size}")
        ...
        let r3 ← expectDecodeStep "decode/raw-0xd702-pldix" r2 pldixInstr 16
        ...
        let fixedNeighborWord : Nat := loadIntFixedWord24 false true false 8
        ...
        let b2 ← expectDecodeStep "decode/raw-boundary-loadint-fixed" b1 (.loadInt false true false 8) 24
        ...
        let rawNeighborBits : BitString :=
          natToBits 0xd701 16 ++ natToBits pldixOpcode 16 ++ addCode.bits
        let n0 := mkSliceFromBits rawNeighborBits
        let n1 ← expectDecodeStep "decode/raw-neighbor-ldux" n0 (.loadIntVar true false false) 16
        let n2 ← expectDecodeStep "decode/raw-neighbor-pldix" n1 pldixInstr 16
        let n3 ← expectDecodeStep "decode/raw-neighbor-tail-add" n2 .add 8
        if n3.bitsRemaining = 0 then
          pure ()
        else
          throw (IO.userError s!"decode/raw-neighbor-end: expected exhausted slice, got {n3.bitsRemaining} bits remaining")

        match assembleCp0 [.loadInt false true false 8] with
    ...
    { name := "unit/dispatch/fallback"
      run := do
        expectOkStack "dispatch/non-loadintvar-add"
          (runPldixDispatchFallback .add #[.null])
          #[.null, intV dispatchSentinel]
        expectOkStack "dispatch/non-loadintvar-fixed-neighbor"
          (runPldixDispatchFallback (.loadInt false true false 8) #[intV 9])
          #[intV 9, intV dispatchSentinel]
        expectOkStack "dispatch/non-loadintvar-slice-neighbor"
          (runPldixDispatchFallback (.loadSliceX true false) #[.cell refLeafA])
          #[.cell refLeafA, intV dispatchSentinel] }
  ]
  oracle := #[
    mkPldixCase "ok/width0-empty-slice" #[.slice oracleSlice0, intV 0],
    mkPldixCase "ok/width0-nonempty-slice" #[.slice oracleSlice13, intV 0],
    mkPldixCase "ok/width1-exact" #[.slice oracleSlice1, intV 1],
    mkPldixCase "ok/width3-of-5" #[.slice oracleSlice5, intV 3],
    mkPldixCase "ok/width8-exact" #[.slice oracleSlice8, intV 8],
    mkPldixCase "ok/width8-with-tail" #[.slice oracleSlice8Tail11, intV 8],
    mkPldixCase "ok/width31-exact" #[.slice oracleSlice31, intV 31],
    mkPldixCase "ok/width64-exact" #[.slice oracleSlice64, intV 64],
    mkPldixCase "ok/width127-exact" #[.slice oracleSlice127, intV 127],
    mkPldixCase "ok/width128-with-tail" #[.slice oracleSlice128Tail17, intV 128],
    mkPldixCase "ok/width255-exact" #[.slice oracleSlice255, intV 255],
    mkPldixCase "ok/width256-exact" #[.slice oracleSlice256, intV 256],
    mkPldixCase "ok/width257-exact" #[.slice oracleSlice257, intV 257],
    mkPldixCase "ok/width257-with-tail" #[.slice oracleSlice257Tail6, intV 257],
    mkPldixCase "ok/width257-min-int257" #[.slice oracleSlice257Min, intV 257],
    mkPldixCase "ok/width257-max-int257" #[.slice oracleSlice257Max, intV 257],
    mkPldixCase "ok/width257-negone-with-tail" #[.slice oracleSlice257NegOneTail6, intV 257],
    mkPldixCase "ok/refs-in-input-slice" #[.slice oracleSliceWithRefs9, intV 4],
    mkPldixCase "ok/deep-stack-preserve-noise"
      #[.null, .cell refLeafB, .slice oracleSlice13, intV 5],

    mkPldixCase "cellund/empty-width1" #[.slice oracleSlice0, intV 1],
    mkPldixCase "cellund/short-4-vs5" #[.slice oracleSlice4, intV 5],
    mkPldixCase "cellund/short-255-vs256" #[.slice oracleSlice255, intV 256],
    mkPldixCase "cellund/short-256-vs257" #[.slice oracleSlice256, intV 257],
    mkPldixCase "cellund/refs-short-6-vs7" #[.slice oracleSliceWithRefs6, intV 7],

    mkPldixCase "range/width-negative" #[.slice oracleSlice13, intV (-1)],
    mkPldixCase "range/width-too-large-258" #[.slice oracleSlice13, intV 258],
    mkPldixCase "range/width-too-large-4096" #[.slice oracleSlice13, intV 4096],
    mkPldixProgramCase "range/width-nan-via-program"
      #[.slice oracleSlice13]
      #[.pushInt .nan, pldixInstr],

    mkPldixCase "underflow/empty" #[],
    mkPldixCase "underflow/one-item-slice" #[.slice oracleSlice13],
    mkPldixCase "underflow/one-item-width" #[intV 0],

    mkPldixCase "type/width-top-null" #[.slice oracleSlice13, .null],
    mkPldixCase "type/width-top-cell" #[.slice oracleSlice13, .cell refLeafA],
    mkPldixCase "type/slice-not-slice-after-valid-width" #[.null, intV 0],
    mkPldixCase "error-order/range-before-slice-type" #[.null, intV 4096],
    mkPldixCase "error-order/width-type-before-slice-type" #[.null, .null],

    mkPldixCase "gas/exact-cost-succeeds"
      #[.slice oracleSlice13, intV 5]
      #[.pushInt (.num pldixSetGasExact), .tonEnvOp .setGasLimit, pldixInstr],
    mkPldixCase "gas/exact-minus-one-out-of-gas"
      #[.slice oracleSlice13, intV 5]
      #[.pushInt (.num pldixSetGasExactMinusOne), .tonEnvOp .setGasLimit, pldixInstr]
  ]
  fuzz := #[
    { seed := 2026021073
      count := 500
      gen := genPldixFuzzCase }
  ]
