...
private def genSdCntLead1FuzzCase (rng0 : StdGen) : OracleCase × StdGen :=
  let (shape, rng1) := randNat rng0 0 6
  if shape = 0 then
    let (total, rng2) := pickBitsMixed rng1
    let (prefOnes, rng3) := randNat rng2 0 total
    let (refs, rng4) := pickRefsMixed rng3
    let s := mkPrefixOneSlice prefOnes total (refsByCount refs)
    (mkSdCntLead1Case s!"fuzz/ok/prefix{prefOnes}/total{total}/refs{refs}" #[.slice s], rng4)
  else if shape = 1 then
    let (total, rng2) := pickBitsMixed rng1
    let (bits, rng3) := randBitString total rng2
    let (refs, rng4) := pickRefsMixed rng3
    let s := mkSliceWithBitsRefs bits (refsByCount refs)
    (mkSdCntLead1Case s!"fuzz/ok/random/len{total}/refs{refs}" #[.slice s], rng4)
    ...
    let program : Array Instr := #[sdskipfirstInstr, sdCntLead1Instr]
    (mkSdCntLead1Case s!"fuzz/ok/cursor/skip{skip}" stack program, rng5)
    ...
    let s := mkPrefixOneSlice prefOnes total (refsByCount refs)
    (mkSdCntLead1Case "fuzz/ok/deep" #[noise, .slice s], rng5)
  else if shape = 4 then
    (mkSdCntLead1Case "fuzz/underflow/empty" #[], rng1)
    ...
    (mkSdCntLead1Case "fuzz/type/top" #[bad], rng2)
  else
    let (bad, rng2) := pickNoiseValue rng1
    (mkSdCntLead1Case "fuzz/type/deep-top" #[.slice (mkPrefixOneSlice 3 8), bad], rng2)
