...
private def genSdPpfxRevFuzzCase (rng0 : StdGen) : OracleCase × StdGen :=
  ...
  if shape = 0 then
    ...
    let (prefLen, rng4) := randNat rng3 0 (sLen - 1)
    let prefBits := sBits.extract 0 prefLen
    let (sRefs, rng5) := pickRefsMixed rng4
    let (prefRefs, rng6) := pickRefsMixed rng5
    let s := Slice.ofCell (Cell.mkOrdinary sBits (refsByCount sRefs))
    let pref := Slice.ofCell (Cell.mkOrdinary prefBits (refsByCount prefRefs))
    (mkSdPpfxRevCase "fuzz/ok/proper-prefix" #[.slice s, .slice pref], rng6)
  else if shape = 1 then
    let (sLen, rng2) := pickBitsMixed rng1
    let (sBits, rng3) := randBitString sLen rng2
    let s := Slice.ofCell (Cell.mkOrdinary sBits #[])
    let pref := Slice.ofCell (Cell.mkOrdinary sBits #[])
    (mkSdPpfxRevCase "fuzz/false/equal" #[.slice s, .slice pref], rng3)
    ...
    let s := Slice.ofCell (Cell.mkOrdinary sBits #[])
    let pref := Slice.ofCell (Cell.mkOrdinary prefBits #[])
    (mkSdPpfxRevCase "fuzz/false/mismatch" #[.slice s, .slice pref], rng4)
  else if shape = 3 then
    let (sLen, rng2) := randNat rng1 0 128
    let (sBits, rng3) := randBitString sLen rng2
    let (extra, rng4) := randNat rng3 1 16
    let prefBits := sBits ++ #[true] ++ stripeBits (extra - 1) 0
    let s := Slice.ofCell (Cell.mkOrdinary sBits #[])
    let pref := Slice.ofCell (Cell.mkOrdinary prefBits #[])
    (mkSdPpfxRevCase "fuzz/false/pref-longer" #[.slice s, .slice pref], rng4)
  else if shape = 4 then
    (mkSdPpfxRevCase "fuzz/underflow/empty" #[], rng1)
  else if shape = 5 then
    let (noise, rng2) := pickNoiseValue rng1
    (mkSdPpfxRevCase "fuzz/underflow/one" #[noise], rng2)
  else if shape = 6 then
    let (bad, rng2) := pickNoiseValue rng1
    (mkSdPpfxRevCase "fuzz/type/top" #[.slice sBasic, bad], rng2)
  else
    let (noise, rng2) := pickNoiseValue rng1
    (mkSdPpfxRevCase "fuzz/ok/deep" #[noise, .slice sBasic, .slice prefBasic], rng2)
