...
def testSgn : IO Unit := do
  ...
  | .halt exitCode st =>
      -- SGN is non-quiet: NaN inputs raise integer overflow (see TON `exec_sgn` + `push_int_quiet`).
      expectExitExc "sgn(nan)" .intOv exitCode
      assert (st.stack.size == 1) s!"sgn(nan): unexpected stack size={st.stack.size}"
      match st.stack[0]! with
      | .int (.num n) => assert (n == 0) s!"sgn(nan): expected 0, got {n}"
      | v => throw (IO.userError s!"sgn(nan): expected 0, got {v.pretty}")
